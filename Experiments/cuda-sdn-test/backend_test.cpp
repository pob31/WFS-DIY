//
// Correctness + timing test for CudaSdnBackend (the GPU Scattering Delay Network
// reverb, CUDA twin of the MetalSdnBackend test). Drives the backend like the
// GpuAsyncPipelineT pump would and validates the streamed output sample-for-
// sample against a CPU reference that mirrors the synchronous
// SDNAlgorithm::processBlock (Source/DSP/ReverbSDNAlgorithm.h).
//
// Identical logic to Experiments/metal-sdn-test/backend_test.mm — only the
// backend type and the build scaffolding differ. Build + run on a Windows /
// NVIDIA box:  build_and_run.bat
//
...
//==============================================================================
// CPU reference — direct transcription of the synchronous SDN per-sample DSP,
// reading its per-path delays/gains/crossfade from a shared SdnHostConfig.
