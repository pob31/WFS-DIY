...
Write-Host "  4. If spatcore/gpu changed (Hip*/kernels/GpuHostWorkPool): refresh the"
Write-Host "     committed prebuilt wfs_hip.dll that release CI bundles -"
Write-Host "     see tools/windows/prebuilt/README.md (it goes stale silently)"
Write-Host "  5. offline-render --path cpu/gpu --check (bit-identical unless the bump"
Write-Host "     is declared value-affecting - then re-baseline DELIBERATELY)"
Write-Host "  6. control-replay: session_roundtrip / osc_replay / mcp_replay"
Write-Host "  7. spatcore standalone tests (reconfigure if CMake changed)"
Write-Host ""
