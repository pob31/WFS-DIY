    ...
    , logger(1000)
{
    ...
    rateLimiter.setSendCallback([this](int targetIndex, const juce::OSCMessage& message)
    {
        ...
        {
            if (auto& conn = connections[static_cast<size_t>(targetIndex)])
            {
                const auto refusalsBefore = conn->getSendErrors();
                const bool sent = conn->send(message);

                // The handshake stall's evidence (see
                // RemoteConnectionState::consecutiveRefusedPings): pings only,
                // and only sends the OS refused, not ones dropped because the
                // connection is down.
                if (message.getAddressPattern().toString() == "/remote/ping")
                {
                    auto& refusedPings = remoteStates[static_cast<size_t>(targetIndex)].consecutiveRefusedPings;
                    if (sent)
                        refusedPings = 0;
                    else if (conn->getSendErrors() != refusalsBefore)
                        ++refusedPings;
                }
...
void OSCManager::applyTargetConfig(int targetIndex, const TargetConfig& config)
{
    ...
    else if (shouldBeConnected)
    {
        if (connectionParamsChanged || !wasConnected)
        {
            ...
            // A Remote target pointed at a new address starts a fresh handshake:
            // the tablet there has had no ping and no dump, and the stall counts
            // so far describe the old address. No onRemoteDisconnected: its
            // /remote/disconnect would go to the new address. The timer pings
            // from Disconnected on its next tick.
            if (config.protocol == Protocol::Remote && connectionParamsChanged)
            {
...
bool OSCManager::connectTarget(int targetIndex)
{
    ...
    // Whatever status this pushes, the Network tab drops a Remote row's stall
    // hint on it, and every Network-tab edit reconnects every enabled target.
    // Re-arm the stall so the next unanswered ping brings the hint back, rather
    // than leaving it gone until a pong or a Tx off/on.
    if (config.protocol == Protocol::Remote)
        remoteStates[static_cast<size_t>(targetIndex)].stallNotified = false;
...
void OSCManager::sendMessage(int targetIndex, const juce::OSCMessage& message)
{
    ...
    const auto& config = targetConfigs[static_cast<size_t>(targetIndex)];
    if (config.protocol == Protocol::Disabled)
        return;
    if (!config.txEnabled)
        return;
...
void OSCManager::sendMessageDirect (int targetIndex, const juce::OSCMessage& message)
{
    if (targetIndex < 0 || targetIndex >= MAX_TARGETS)
    {
        DBG ("OSCManager::sendMessageDirect - invalid target index: " << targetIndex);
        return;
    }
        ...
        else if (config.protocol == Protocol::Remote && config.txEnabled)
        {
            switch (remoteState.phase)
            {
                case RemoteConnectionState::Phase::Disconnected:
                    // Start connection attempt. The refused-ping count restarts
                    // with it, so the stall judges this attempt's pings alone;
                    // stallNotified is re-armed so a retry after Tx off/on can
                    // warn again (the UI clears its flag on the status change).
                    remoteState.consecutiveRefusedPings = 0;
                ...
                case RemoteConnectionState::Phase::Connecting:
                    ...
                    if (now - remoteState.lastPingSentTime >= HEARTBEAT_INTERVAL_MS)
                    {
                        ...
                        if (++remoteState.pingAttemptsWhileConnecting >= STALL_PING_ATTEMPTS)
                        {
                            ...
                            // yet, hence the -1.
                            const bool sendsFailing =
                                remoteState.consecutiveRefusedPings >= STALL_PING_ATTEMPTS - 1;

                            // Once per attempt, then only to take a sendsFailing verdict
                            // back: a ping that got out disproves it (Local Network
                            // re-allowed while the tablet is still unreachable). Never
                            // the other way round: refusals that begin mid-attempt are
                            // more likely macOS refusing a host that stopped answering
                            // ARP (EHOSTDOWN, for a while) than a permission revoked.
                            if (! remoteState.stallNotified
                                || (remoteState.stallSendsFailing && ! sendsFailing))
                            {
                ...
                case RemoteConnectionState::Phase::Connected:
                    ...
                    {
                        // Timeout - disconnect and try to reconnect. A new attempt,
                        // so its refused-ping count starts afresh too.
                                       ...
                                       ConnectionMode transport)
{
    ...
    {
        handleClusterCumulativeScaleRotationMessage(message);
    }
...
void OSCManager::handleClusterCumulativeScaleRotationMessage(const juce::OSCMessage& message)
{
    ...
    if (parsed.cumulativeScale == 0.0f)
    {
        ...
        {
            clusterGestureSnapshots.erase(cid);
        });
        return;
    }
    ...
    clusterGestureScale.store(parsed.cumulativeScale);
    clusterGestureRotation.store(parsed.cumulativeRotation);
    clusterGestureActive.store(true);
...
bool OSCManager::hasQLabTarget() const
{
    for (int i = 0; i < MAX_TARGETS; ++i)
    {
        if (targetConfigs[static_cast<size_t>(i)].protocol == Protocol::QLab)
            return true;
    }
    return false;
}

int OSCManager::getQLabPatchNumber() const
{
    for (int i = 0; i < MAX_TARGETS; ++i)
    {
        if (targetConfigs[static_cast<size_t>(i)].protocol == Protocol::QLab)
            return targetConfigs[static_cast<size_t>(i)].qlabPatchNumber;
    }
    return 1;
}

void OSCManager::sendToQLab (const QLabCueSequence& sequence,
                              std::function<void(int sentCount)> onComplete)
{
    if (sequence.groupMessages.empty() && sequence.networkCues.empty())
    {
        if (onComplete)
    ...
    auto targetsCopy = std::make_shared<std::vector<int>> (qlabTargets);
    auto completeCb = std::make_shared<std::function<void(int)>> (std::move (onComplete));
    auto* manager = this;

    std::thread ([manager, seqCopy, targetsCopy, completeCb]()
    {
        ...
        // Listener for QLab replies (fires on OSCReceiver's network thread)
        struct ReplyListener : public juce::OSCReceiver::Listener<juce::OSCReceiver::RealtimeCallback>
        {
            juce::String* uid;
        ...
        listener.uid = &replyUID;
        listener.lock = &replyLock;
        listener.event = &replyEvent;

        juce::OSCReceiver replyReceiver;
        replyReceiver.addListener (&listener);

        if (!replyReceiver.connect (DEFAULT_QLAB_REPLY_PORT))
        {
        ...
        auto sendMsg = [&](const juce::OSCMessage& msg)
        {
            for (int targetIdx : *targetsCopy)
                manager->sendMessageDirect (targetIdx, msg);
            ++sentCount;
            juce::Thread::sleep (delayBetweenMs);
        };

        // Helper lambda: query unique ID from QLab and wait for reply
        auto queryUniqueID = [&]() -> juce::String
        {
            {
            ...
            sendMsg (juce::OSCMessage ("/cue/selected/uniqueID"));

            if (replyEvent.wait (queryTimeoutMs))
            {
                const juce::ScopedLock sl (replyLock);
                return replyUID;
            }
