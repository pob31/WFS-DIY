...
inline ToolResult getGlobalState (WFSValueTreeState& state)
{
    auto root = std::make_unique<juce::DynamicObject>();

    auto addSection = [&] (const juce::String& key, juce::ValueTree section)
    {
        if (section.isValid())
            root->setProperty (key, detail::subtreeToVar (section));
    };

    addSection ("config",   state.getConfigState());
    addSection ("network",  state.getNetworkState());
    addSection ("admosc",   state.getADMOSCState());
    addSection ("tracking", state.getTrackingState());
    addSection ("stage",    state.getStageState());
    addSection ("master",   state.getMasterState());
    addSection ("io",       state.getIOState());
    addSection ("show",     state.getShowState());
    addSection ("binaural", state.getBinauralState());
...
inline ToolDescriptor describeGlobalState (WFSValueTreeState& state)
{
    ...
    d.description = "Read-only dump of every NON-channel parameter: stage shape "
                    "and dimensions, origin offsets (originWidth/Depth/Height), "
                    "master level / system latency / Haas, network globals "
                    "(interface, ports, OSC Query), tracking globals, ADM-OSC "
                    "mappings, binaural settings, IO and show config. Use this "
                    "to discover what's currently set globally before issuing "
                    "writes. Complements session_get_state (per-channel positions) "
                    "and session_get_channel_full (one channel, every parameter).";
