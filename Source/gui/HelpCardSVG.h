...
inline std::unique_ptr<juce::Drawable> parseSignalFlow(const juce::String& svg)
{
    ...
    {
        // Light mode: explicit colors for visibility on light/white background

        // st16 texts (no fill) → force black
        adapted = adapted.replace("translate(60.24 131.6)\">",  "translate(60.24 131.6)\" fill=\"#000\">")
                         .replace("translate(169.23 738.6)\">", "translate(169.23 738.6)\" fill=\"#000\">")
                         .replace("translate(446 921.07)\">", "translate(446 921.07)\" fill=\"#000\">");

        // Strokes: ensure dark and visible
        adapted = adapted.replace("stroke: #000", "stroke: #333");
        adapted = adapted.replace("stroke: #fff", "stroke: #333");

        // White fills → light grey (so shapes are visible on white bg)
        // But keep WFS processor text white (it's on a dark box)
        // Strategy: change fill: #fff to #e0e0e0, then restore st10 text
        adapted = adapted.replace("fill: #fff", "fill: #e0e0e0");

        // Dark box fills stay dark for contrast
        adapted = adapted.replace("fill: #333", "fill: #444");

        // Blue: darken slightly for light bg
        adapted = adapted.replace("fill: blue", "fill: #0033aa");
...
/** Parse the map pin SVG with theme-adapted dot color */
inline std::unique_ptr<juce::Drawable> parseMapPin()
{
    juce::String svg = R"SVG(<?xml version="1.0" encoding="UTF-8" standalone="no"?>
  ...
  <g
    ...
    <g
      ...
      <path
          ...
          style="stroke:#5a0000;stroke-width:5.7;fill:#f41922"
