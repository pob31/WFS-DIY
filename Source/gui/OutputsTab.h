...
private:
    ...
    void setupPositionTab()
    {
        ...
        // Initialise output-parameter Tab circuits (one loop per column)
        outputParamCircuits = {
            // Left column value labels
            { &attenuationValueLabel, &delayLatencyValueLabel, &distanceAttenValueLabel },
            // Coordinate position editors
        ...
        circuitTabHandler.circuits = &outputParamCircuits;
    }
    ...
    // Output Parameters tab circuits for Tab navigation (one loop per column)
    std::vector<std::vector<juce::Component*>> outputParamCircuits;
    ...
    // and directly shows/focuses the next circuit member within the same column.
    ...
    static void drawSpeakerIcon(juce::Graphics& g, float cx, float cy, float orientDeg, float size,
                                 juce::Colour fillColour, juce::Colour strokeColour)
    {
        float orientRad = juce::degreesToRadians(orientDeg);
        float dirX = -std::sin(orientRad);
        float dirY = -std::cos(orientRad);
        float perpX = -dirY, perpY = dirX;
        float h = size, bw = size * 0.875f, fw = size * 0.46f;

        float fcx = cx + dirX * h * 0.5f, fcy = cy + dirY * h * 0.5f;
        float bcx = cx - dirX * h * 0.5f, bcy = cy - dirY * h * 0.5f;
        float flx = fcx + perpX * fw * 0.5f, fly = fcy + perpY * fw * 0.5f;
        float frx = fcx - perpX * fw * 0.5f, fry = fcy - perpY * fw * 0.5f;
        float blx = bcx + perpX * bw * 0.5f, bly = bcy + perpY * bw * 0.5f;
        float brx = bcx - perpX * bw * 0.5f, bry = bcy - perpY * bw * 0.5f;

        juce::Path keystone;
        keystone.startNewSubPath(blx, bly);
        keystone.lineTo(flx, fly);
        keystone.lineTo(frx, fry);
        keystone.lineTo(brx, bry);
        keystone.closeSubPath();
        g.setColour(ColorScheme::get().background);
        g.fillPath(keystone);
        g.setColour(strokeColour);
        g.strokePath(keystone, juce::PathStrokeType(1.2f));

        float mh = h * 0.55f;
        float mtx = bcx + dirX * mh, mty = bcy + dirY * mh;
        juce::Path mem;
        mem.startNewSubPath(blx, bly);
        mem.lineTo(mtx, mty);
        mem.lineTo(brx, bry);
        mem.closeSubPath();
        ...
        g.fillPath(mem);
        g.setColour(strokeColour);
        g.strokePath(mem, juce::PathStrokeType(0.8f));
    }

    /** Helper: draw a listener (simple head + shoulders silhouette) */
    static void drawListener(juce::Graphics& g, float cx, float cy, float size, juce::Colour colour)
    {
        g.setColour(colour);
        float headR = size * 0.3f;
        g.drawEllipse(cx - headR, cy - size * 0.5f, headR * 2, headR * 2, 1.5f);
        // Shoulders arc
        juce::Path shoulders;
        shoulders.addArc(cx - size * 0.5f, cy, size, size * 0.6f, 0.0f, juce::MathConstants<float>::pi, true);
        g.strokePath(shoulders, juce::PathStrokeType(1.5f));
    }

    /** Helper: draw an input source (circle with dot) */
    static void drawInputSource(juce::Graphics& g, float cx, float cy, float size, juce::Colour colour)
    {
        g.setColour(colour);
    ...
    void buildOutputHelpContent()
    {
        ...
        outputHelpCard.addIllustration(220, [](juce::Graphics& g, juce::Rectangle<int> area) {
            ...
            float gap = 6.0f;
            float panelW = (totalW - gap * 2) / 3.0f;

            for (int step = 0; step < 3; ++step)
            {
                float px = (float)area.getX() + step * (panelW + gap);
                float py = (float)area.getY();
                const float s = panelW / 950.0f;
                auto sx = [&](float v) { return px + v * s; };
                auto sy = [&](float v) { return py + v * s; };
                ...
                auto drawList = [&](float cx, float cy) {
                    ...
                    g.setColour(cs.textPrimary); g.drawEllipse(sx(cx)-8*s, sy(cy)-8*s, 16*s, 16*s, 2*s);
                };

                // Stage polyline (thick black angled line)
                { juce::Path stg; stg.startNewSubPath(sx(932), sy(338)); stg.lineTo(sx(380), sy(608)); stg.lineTo(sx(36), sy(608));
                  g.setColour(cs.textPrimary); g.strokePath(stg, juce::PathStrokeType(8 * s)); }

                // Top speaker (near-field, rotated -66.78)
                drawSpk(255.58f, 156.79f, -66.78f, 278.36f, 163.22f, 267.32f, 167.63f, 271.73f, 178.68f);
                // Bottom speaker (at stage edge, rotated -90)
                drawSpk(240.58f, 580.79f, -90, 260.87f, 582.55f, 252.45f, 590.96f, 260.87f, 599.38f);
                // Source
                drawSrc(72.59f, 470.59f);

                if (step == 0)
                {
                    // Tuning 1: green dashed near-field→listener, blue source→listener, X on top speaker
                    drawList(403.19f, 530);
                    drawDashed(264, 584, 392, 533, 8, 4, juce::Colour(0xFF009245));
                    drawDashed(385.5f, 527.5f, 89.13f, 475.54f, 8, 4, juce::Colours::blue);
                    // Cross on top speaker
                    drawSolid(305.1f, 155.1f, 322.9f, 225.9f, 8, juce::Colour(0xFF666666));
                    drawSolid(278.6f, 199.4f, 349.4f, 181.6f, 8, juce::Colour(0xFF666666));
                }
                else if (step == 1)
                {
                    // Tuning 2: red dashed top→listener(far), blue source→listener, X on bottom speaker
                    drawList(848.19f, 306);
                    drawDashed(276, 172, 837, 303, 8, 4, juce::Colours::red);
                    drawDashed(821, 310, 89.1f, 465.46f, 8, 4, juce::Colours::blue);
                    // Cross on bottom speaker
                    drawSolid(326.02f, 537.76f, 289.98f, 601.24f, 8, juce::Colour(0xFF666666));
                    drawSolid(276.26f, 551.48f, 339.74f, 587.52f, 8, juce::Colour(0xFF666666));
                }
                else
                {
                    // Tuning 3: both speakers active, red+green→listener, blue source→listener
                    drawList(568.19f, 436);
                    drawDashed(276, 172, 557, 433, 8, 4, juce::Colours::red);
                    drawDashed(264, 588, 553, 444, 8, 4, juce::Colour(0xFF009245));
                    drawDashed(89.54f, 469.08f, 548, 436, 8, 4, juce::Colours::blue);
                }

                // Step label
                g.setColour(cs.textSecondary);
                g.setFont(juce::FontOptions(juce::jmax(8.0f, 10.0f * s * 950.0f / panelW)).withStyle("Bold"));
                g.drawText(juce::String("Step ") + juce::String(step + 1),
                           (int)px, (int)((float)area.getY() + totalH), (int)panelW, 14, juce::Justification::centredTop);
            }
            g.restoreState();
        });
