...
**Every top-level window follows Screen Sharing** (`Source/gui/ScreenShareRendering.h`). On Windows JUCE 9 draws each window with Direct2D through DirectComposition, which never paints the window's GDI surface, so single-window capture (Zoom's window share) shows the first frame forever. System Config's Screen Sharing toggle (session only, Windows only) switches every window to the software renderer and opens menus inside their window (`WfsLookAndFeel::getParentComponentForMenuOptions`). A new window calls `ScreenShareRendering::apply (*this)` before its first `setVisible (true)`; a dialog is built with `DialogWindow::LaunchOptions::create()` + `apply` + `enterModalState (true, nullptr, true)` instead of `launchAsync()`, which shows it before returning; a directly built `AlertWindow` calls `apply` before `enterModalState`. Popup menus and `AlertWindow::showAsync` alerts are covered in `WfsLookAndFeel`, and a 4 Hz watcher switches any window that missed its call while the mode is on. A popup menu wants a target component (`withTargetComponent`) to open inside its window.
...
**Webcam head tracking** (`tools/headtrack/`, see its README): a standalone plugin DLL (`wfs_headtrack.dll` — OpenCV capture + YuNet 5-landmark detection + solvePnP), loaded through the versioned C ABI `spatcore/binaural/plugin/HeadTrackPluginApi.h` — the GPU-plugin split applied to CV so OpenCV never touches the app build. App side: `Source/DSP/CameraHeadTrackerSource.h` (dlopen + ABI check, zero-calibration by matrix pre-multiplication, per-angle 1-Euro filtering, 300 ms staleness → manual fallback), registered by `HeadTrackerManager` as "Webcam"; camera opens on selection, releases on deselection; **Set Zero** button calibrates while looking at the stage. Camera index: `headtrackCameraIndex` in the machine-local settings file. Built by `tools/headtrack/build-headtrack-plugin.ps1/.sh` (CI compile-checks all 3 OSes; Windows release stages it + OpenCV runtime + model into the installer, all `skipifsourcedoesntexist`).
...
- SDN: **chunked** node-parallel. `readBasePos` is frozen at block start, and the block is swept in windows of C samples where C = `min over paths of min(D, MAX_DELAY − D)` using each path's *effective* delay (the shorter tap while crossfading). Within such a window no node can read a cell another node writes, so nodes are independent; C = 1 degenerates to the old per-sample lockstep. Bit-exact and worker-count invariant (verified by the `reverb-sdn` goldens). **The earlier claim that "minimum delay ~70 samples provides natural temporal separation" was wrong** — 70 < the 256-sample internal block, which is exactly why the naive node-parallel sweep was racy and got reverted in `c3e7ae2`; the bound above is what actually makes it safe.
  - **GPU (CUDA) mirrors this**, with one structural difference: `__syncthreads()` orders only a single block, so a chunk boundary must be a **kernel-launch** boundary. `sdn_process_nodes` runs `grid = numNodes, block = 32` (one block per node, spreading across SMs) and the host picks it per block from `SdnHostConfig::chunkSamplesForBlock`; the original `grid = 1` `sdn_process` lockstep is retained for geometries too tight for a parallel window, and for the pre-geometry state where every delay is still 1. Both kernels are bit-exact with each other and with the CPU. **HIP and Metal now carry the same reshape**: HIP in `c12f3b6` (it compiles `CudaSdnKernels.h` verbatim, so the port was almost entirely host-side) and Metal in `a2f1ae4` (`numNodes` threadgroups × 32 lanes). Both retain the lockstep path for tight geometry and both honour `WFS_SDN_NODE_PARALLEL=0` and `WFS_SDN_TRACE=1`.
...
- **Cooldown** - no long press can arm within 3 s (`longPressCooldownMs`) of the last marker edit (`lastMarkerEditMs`, stamped by `noteMarkerEdited()`): a drag that left the tolerance, a second-finger edit, the release of the finger that held the marker for one, a double-click offset reset, or a wheel (stereo axis) / arrow-key / PageUp-Down / L edit of the selection. An edit made while a press is held also cancels that press. Grabbing a marker again to fine-tune it never leaves the map.
...
1. **MCP Server hardening**: the server itself is built and shipping — the generator pipeline (`tools/generate_mcp_tools.py` → `generated_tools.json`), ~416 registered tools, the three-tier confirmation model, AI-change undo/redo, 16 knowledge resources and 6 workflow prompts are all in place, and the protocol core lives in spatcore (`spatcore/control/mcp/`). Specs under `Documentation/MCP/`; the reverse-engineered map is `docs/architecture/control-plane-map.md` §5. **Remaining:** there is still **no authentication** on port 7400 — CORS is the only guard, and it does not constrain non-browser clients, so loopback binding is the real boundary. The tier-2 confirm token stops accidents, not an attacker, since a caller simply reads the token from its own response and re-calls; only tier-3 needs a human. Adopting MCP spec revision 2026-07-28 (stateless core, `server/discover`, `resultType`, MRTR — which the tier-2 confirm handshake already resembles) is the other open item.
