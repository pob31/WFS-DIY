...
*Session changes: CPU SDN made chunked node-parallel (0.935→0.460 ms/block at 32 nodes, bit-exact — `setParallelFor` had been an empty override); CUDA SDN kernel reshaped to `grid = numNodes, block = 32` with the incoming vector moved out of a local-memory-spilled `float[32]` into shared memory (32 nodes: 5070 4.52→0.319 ms, T4 7.01→0.426 ms median — 14-16x, bit-exact, T4 from 131% of budget to 8%; the `grid = 1` lockstep is retained for geometries too tight for a parallel window); HIP ported (compile-verified only — no AMD GPU on this rig; it was a SILENT struct-layout mismatch, since HIP compiles CudaSdnKernels.h verbatim), **Metal still NOT ported** *(superseded — Metal landed later in `a2f1ae4`)*; SDN `prepare()` warmup through both kernels. Reverb nodes now default to a semi-ellipse/ring at 1.5x the stage, 2 m high, deterministically jittered and de-crowded in Z (was a collinear 1 m line at Z = 0 — the source of the degenerate SDN geometry); ownership rule + resize/count hooks NOT yet wired. Level-meter GPU strip now shows for GPU reverb on a CPU direct path and shares the controls row instead of shifting the meters. Position ownership rule wired (persisted one-way `positionsUserOwned` latch: stage/count changes re-run the initial input+reverb placement until the user opens the Map tab or edits any position, then never again); Ctrl-mirror axis fixed to the stage centre with a merge guard at all three sites; auto-layout reuses the same Z de-crowding. **Measured and corrected a false contract**: CPU and GPU SDN are NOT bit-identical (94% of samples differ, max 2.8e-7 / −123 dBFS, RMS 5.7e-8 — float32 association, and sm_120 vs sm_75 disagree too, which is why GPU goldens are per-device); the real contract is float32-equivalence, and the CPU lockstep was never buying the parity that justified it. Metal SDN port remains open — see `docs/Metal_SDN_NodeParallel_Handoff.md`. *(Since closed: ported in `a2f1ae4` and validated; as of v1.0.0beta39 all three vendors ship node-parallel SDN.)* GPU latency work on the RTX 5070 + Tesla T4 rig — new pipeline-bench harness (first tool to exercise the real GpuAsyncPipelineT pump/rings/underrun path), opt-in CUDA graph submission for the WFS gather backend (WFS_GPU_GRAPHS=1; bit-exact, T4 depth-1 underruns 82→5 per 30 s, neutral on WDDM 5070), Windows_GPU_Latency_Tuning.md runbook with measured depth sweeps + roadmap (OB graphs, HIP port, kernel occupancy split); OB scatter backend got a prepare-time warmup launch (25-85 ms first-block lazy-init stall killed; bit-exact, goldens pass). Found + fixed a stale staged wfs_cuda.dll (predated FR-diffusion + SDN-gain commits); re-recorded per-device GPU goldens; audio-engine-map doc drift (blocking-sync event vs cudaStreamSynchronize) fixed in both mirrors.*
*JUCE Version: 9.0.0*
