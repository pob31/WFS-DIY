...
- **`TestSignalGenerator` moved to `spatcore/io/`** (`Source/DSP/TestSignalGenerator.h` is now a `using`-alias shim, and the `.cpp` is gone). Its 500 ms fade-in on the continuous signals is hearing protection, not polish — a rig under test can be pointed at someone's head at full system gain — and is unchanged and now regression-tested. It gained an opt-in `setDeterministicSeed()` for reproducible renders.
