...
- **`midiTooltip`**
  ...
  - ES: Solo para recuperar snapshots. Este puerto se abre únicamente como entrada y no se usa para nada más: WFS-DIY no le envía MIDI, y los únicos mensajes a los que responde son los note-on que recuperan un snapshot (entradas y efectos). Asigne un canal y una nota a un snapshot en la ventana Edit Scope de la pestaña Inputs o Effects; un note-on con velocidad superior a 64 lo recupera.
  - [ ] OK    Fix: 
...
- **`unpatchInputsMessage`**
  ...
  - ES: ¿Está seguro de que desea eliminar todos los enrutamientos de entrada?
  - [ ] OK    Fix: 

- **`unpatchInputsTitle`**
  - EN: Unpatch All Inputs
  - ES: Desconectar todas las entradas
  - [ ] OK    Fix: 

- **`unpatchOutputsMessage`**
  - EN: Are you sure you want to remove all output patches?
  - ES: ¿Está seguro de que desea eliminar todos los enrutamientos de salida?
  - [ ] OK    Fix: 

- **`unpatchOutputsTitle`**
  - EN: Unpatch All Outputs
  - ES: Desconectar todas las salidas
  - [ ] OK    Fix: 
...
- **`referenceMode`**
  ...
  - ES: Seleccione el punto de referencia para las transformaciones del cluster: First Input, Baricentro o Shared Position (todos los miembros coinciden; la escala y la rotación se aplican a los offsets individuales).
  - [ ] OK    Fix: 
...
- **`exportTitle`**
  ...
  - ES: Exportar configuración de efectos
  - [ ] OK    Fix: 

- **`importTitle`**
  - EN: Import Effects Configuration
  - ES: Importar configuración de efectos
  - [ ] OK    Fix: 
...
- **`editOnMap`**
  ...
  - ES: Activar la edición de las posiciones de los efectos desde la pestaña Map.
  - [ ] OK    Fix: 
...
- **`groupMute`**
  ...
  - ES: Pulsación larga para silenciar o reactivar a la vez todos los miembros de este grupo de enlace.
  - [ ] OK    Fix: 
...
- **`linkGroup`**
  ...
  - ES: Enlazar este efecto a un grupo para que las ediciones de módulos se propaguen a los demás miembros.
  - [ ] OK    Fix: 

- **`linkMode`**
  - EN: How this channel takes group edits: off, absolute, or relative.
  - ES: Cómo recibe este canal las ediciones del grupo: desactivado, absoluto o relativo.
  - [ ] OK    Fix: 

- **`loopGuard`**
  - EN: The loop guard has muted this channel's effect-to-effect feed.
  - ES: La protección de bucle ha silenciado la alimentación de efecto a efecto de este canal.
  - [ ] OK    Fix: 
...
- **`solo`**
  ...
  - ES: Pulsación larga para poner en solo este retorno de efecto.
  - [ ] OK    Fix: 

- **`soloEffects`**
  - EN: Long-press to solo the effect returns: the direct signal is muted.
  - ES: Pulsación larga para poner en solo los retornos de efectos: la señal directa se silencia.
  - [ ] OK    Fix: 
...
- **`cleared`**
  ...
  - ES: Cadena de efectos vaciada.
  - [ ] OK    Fix: 

- **`clearedAll`**
  - EN: Every effect chain cleared.
  - ES: Todas las cadenas de efectos vaciadas.
  - [ ] OK    Fix: 

- **`configLoaded`**
  - EN: Effects configuration loaded.
  - ES: Configuración de efectos cargada.
  - [ ] OK    Fix: 

- **`configSaved`**
  - EN: Effects configuration saved.
  - ES: Configuración de efectos guardada.
  - [ ] OK    Fix: 
...
- **`body`**
  ...
  - ES: Las herramientas de diagnóstico están ocultas por defecto: una pulsación larga sobre el botón Diagnostics las muestra u oculta. Aparecen automáticamente si la sesión anterior no se cerró correctamente.\nPara enviar comentarios o informar de un problema, haga clic en «Reportar problema» (Report Issue): se abre el gestor de incidencias de GitHub del proyecto en su navegador. Describa qué ocurrió, qué esperaba y los pasos para reproducirlo, y adjunte los datos de diagnóstico exportados.\n«Exportar registros» (Export Logs) copia los datos útiles en una carpeta WFS-DIY-logs en la ubicación que elija: los registros de la sesión actual y de hasta cinco sesiones anteriores, más el archivo de ajustes de la aplicación. Adjunte esta carpeta (o un zip de ella) a su informe.\nLos registros de sesión contienen información de arranque (versión de la aplicación, sistema operativo, CPU, número de canales), la carga de proyectos, la actividad de red y los errores. Nunca se graba audio.\n«Abrir carpeta de registros» (Open Log Folder) muestra los registros en bruto en el disco (carpeta WFS-DIY/logs de los datos de aplicación del usuario), útil para localizar una sesión concreta.\n«Copiar info del sistema» (Copy System Info) coloca un breve resumen en el portapapeles — versión de la aplicación, sistema operativo, CPU y el dispositivo de audio actual con su frecuencia de muestreo y tamaño de búfer — listo para pegar en una incidencia.
  - [ ] OK    Fix: 

- **`title`**
  - EN: Diagnostics & Feedback
  - ES: Diagnóstico y comentarios
  - [ ] OK    Fix: 
...
- **`body`**
  ...
  - ES: Un canal de efectos es una cadena de once módulos (distorsión, dos EQ, dos dinámicas, chorus/flanger, phaser, trémolo, reverberación, delay multitap, bitcrusher) alimentada por las entradas y por otros efectos a través de la matriz de envíos. Su salida es un RETORNO situado en el escenario como una entrada: se reproduce por los arreglos WFS desde su propia posición, con la misma orientación, ley de atenuación y mutes que una entrada.\nLa cabecera replica la de la pestaña Reverb: selector de canal, nombre, visibilidad en el mapa, Edit on Map, Solo Effects, Mute, Solo y Clear de emergencia (pulsación larga; mantenga Ctrl para vaciar todas las cadenas). Los tres indicadores dan el estado del motor para este canal: protección de bucle activada, parte de un bucle de realimentación, alimentado por una entrada (punto de entrada de su conjunto).\nLos grupos de enlace no son como las reverbs, que forman un único sistema: un canal de un grupo comparte su cadena (orden, bypass, parámetros de los módulos) con los demás miembros según el modo de enlace de cada uno —Absolute copia, Relative conserva la diferencia, Off no recibe nada—, mientras que sus movimientos, envíos, mutes y posición nunca se propagan. Arrastre con Ctrl cualquier control para editar solo este canal. Mute Group silencia una vez a cada miembro y deja a cada uno editable por separado.
  - [ ] OK    Fix: 
...
- **`body`**
  ...
  - ES: - Clic izquierdo en una entrada o cluster permite moverlo arrastrando.\n- Clic izquierdo con Mayús añade o quita entradas de la selección.\n- Clic izquierdo arrastrado dibuja un rectángulo de selección.\n- Doble clic restablece el desplazamiento de posición.\n- Clic largo o pulsación larga (0,7 a 1,2 s) sin movimiento cambia, al soltar, a la pestaña de la entrada seleccionada. Alejarse lo cancela aunque luego se vuelva, y se ignora durante 3 s tras mover o editar un marcador.\n- Clic fuera de cualquier entrada limpia la selección.\n- Clic derecho arrastrado desplaza la vista del mapa. Arrastre con dos dedos también.\n- La rueda del ratón hace zoom. Pellizco con dos dedos también.\n- Con entradas estéreo seleccionadas, la rueda del ratón gira su eje de imagen en lugar de hacer zoom: 5° por muesca, 1° con Shift. Mantén Ctrl/Cmd para hacer zoom como siempre.\n- Clic del botón central restablece la vista.\n- Las flechas mueven X/Y, RePág/AvPág la altura.\n- Un segundo dedo puede rotar la directividad y ajustar la altura.\n- En clusters, un segundo dedo puede rotar y escalar.\n- En una entrada estéreo tocada, con Shift pulsado el segundo dedo actúa sobre la imagen estéreo: rotar cambia el eje de la imagen y pellizcar cambia la anchura.\n- El botón situado debajo del que muestra los niveles de audio suspende todas las ediciones con el segundo dedo en entradas y clusters. El desplazamiento de la vista, el zoom y el movimiento en simetría de los pares de nodos de reverberación siguen funcionando. Su estado no se guarda.\n- Con un Space Mouse, manteniendo Shift con una entrada estéreo seleccionada, el giro cambia el eje de la imagen y tirar/empujar cambia la anchura, en el mismo sentido que el segundo dedo: tirar hacia arriba amplía, empujar hacia abajo estrecha. Shift con XY sigue moviendo la entrada.\n- Pulsar L con entradas estéreo seleccionadas bloquea o desbloquea la orientación de su imagen. Bloqueada, el par se abre a izquierda/derecha de sala girado solo por el valor de Eje, y ya no sigue a la fuente alrededor del origen.\n- Las entradas, arrays de salida y nodos de reverberación pueden ocultarse.\n- Las entradas pueden bloquearse.\n- Los nodos de reverberación pueden moverse. Ctrl/Cmd mueve pares en simetría.\n- El radio del Live Source Tamer se muestra cuando está activado.\n- Los niveles de audio pueden mostrarse en el mapa.
  - [ ] OK    Fix: 
...
- **`body`**
  ...
  - ES: Los snapshots almacenan parámetros de entrada y de efectos, pero pueden tener un alcance para ser recuperados durante una actuación.\nEl Alcance indica al sistema qué datos almacenar o recuperar.\nHay varios métodos disponibles:\n- Registrar solo los datos necesarios en archivos locales. El filtro se aplica al almacenar.\n- Registrar todos los datos y un filtro en archivos locales. El filtro se aplica al recuperar.\n- Registrar todos los datos en cues de QLab. No recomendado para configuraciones grandes.\nEl alcance puede mostrar y preseleccionar automáticamente los parámetros modificados manualmente. Los cambios se marcan en amarillo.\nUn snapshot contiene las entradas y los canales de efectos. La ventana de alcance tiene una pestaña para las entradas y otra para los efectos, una cuadrícula por familia; en la cuadrícula de efectos, cada módulo de la cadena es una fila, con sus bandas y taps. Las pestañas Entradas y Efectos muestran la misma fila de snapshots: un snapshot guardado o recuperado desde cualquiera de ellas abarca ambas, y una nota MIDI o un cue OSC recupera ambas.
  - [ ] OK    Fix: 

- **`title`**
  - EN: Snapshots and Scope
  - ES: Snapshots y Alcance
  - [ ] OK    Fix: 
...
- **`snapshotIdentity.fixNumbers`**
  ...
  - ES: Corregir números primero
  - [ ] OK    Fix: 

- **`snapshotIdentity.intro`**
  - EN: Snapshot '{name}' was stored when these channels were on other hardware inputs:
  - ES: El snapshot «{name}» se guardó cuando estos canales estaban en otras entradas de hardware:
  - [ ] OK    Fix: 
...
- **`snapshotIdentity.noFix`**
  ...
  - ES: No se encontró una correspondencia de hardware uno a uno, así que no se ofrece renumeración automática. Si se recableó a propósito, continúe; si no, cancele y revise la lista de canales.
  - [ ] OK    Fix: 
...
- **`snapshotIdentity.proceedNote`**
  ...
  - ES: Continuar aplica el snapshot por número de canal tal cual. Cancelar no cambia nada.
  - [ ] OK    Fix: 

- **`snapshotIdentity.title`**
  - EN: Snapshot stored under a different patching
  - ES: Snapshot guardado con otro patch
  - [ ] OK    Fix: 
...
- **`attenuationLawButton`**
  ...
  - ES: Modelo de Attenuation Law (disminución lineal del volumen con la distancia entre objeto y altavoz, o cuadrática).
  - [ ] OK    Fix: 
...
- **`deleteSnapshot`**
  ...
  - ES: Eliminar el snapshot seleccionado con confirmación.
  - [ ] OK    Fix: 
...
- **`frActiveButton`**
  ...
  - ES: Activar las Floor Reflections simuladas para el objeto.
  - [ ] OK    Fix: 

- **`frAttenuationSlider`**
  - EN: Attenuation of the Simulated Floor Reflections for the Object.
  - ES: Atenuación de las Floor Reflections simuladas para el objeto.
  - [ ] OK    Fix: 

- **`frDiffusionDial`**
  - EN: Diffusion Effect of the Simulated Floor Reflections for the Object.
  - ES: Efecto de difusión de las Floor Reflections simuladas para el objeto.
  - [ ] OK    Fix: 

- **`frHighShelfActiveButton`**
  - EN: Enable High Shelf Filter for Floor Reflections.
  - ES: Activar el filtro High Shelf para las Floor Reflections.
  - [ ] OK    Fix: 

- **`frHighShelfFreqSlider`**
  - EN: High Shelf Frequency for Floor Reflections.
  - ES: Frecuencia del High Shelf para las Floor Reflections.
  - [ ] OK    Fix: 

- **`frHighShelfGainSlider`**
  - EN: High Shelf Gain for Floor Reflections.
  - ES: Ganancia del High Shelf para las Floor Reflections.
  - [ ] OK    Fix: 

- **`frHighShelfSlopeSlider`**
  - EN: High Shelf Slope for Floor Reflections.
  - ES: Pendiente del High Shelf para las Floor Reflections.
  - [ ] OK    Fix: 

- **`frLowCutActiveButton`**
  - EN: Enable Low Cut Filter for Floor Reflections.
  - ES: Activar el filtro Low Cut para las Floor Reflections.
  - [ ] OK    Fix: 

- **`frLowCutFreqSlider`**
  - EN: Low Cut Frequency for Floor Reflections.
  - ES: Frecuencia del Low Cut para las Floor Reflections.
  - [ ] OK    Fix: 
...
- **`lsPeakEnableButton`**
  ...
  - ES: Activar o desactivar el compresor rápido (pico) para el Live Source Tamer.
  - [ ] OK    Fix: 
...
- **`lsSlowEnableButton`**
  ...
  - ES: Activar o desactivar el compresor lento para el Live Source Tamer.
  - [ ] OK    Fix: 
...
- **`minimalLatencyButton`**
  ...
  - ES: Elegir entre Acoustic Precedence y Minimal Latency para la precedencia de amplificación.
  - [ ] OK    Fix: 
...
- **`muteMacrosSelector`**
  ...
  - ES: Mute Macros para silenciar y reactivar rápidamente los arrays.
  - [ ] OK    Fix: 
...
- **`reloadSnapshot`**
  ...
  - ES: Recargar el snapshot seleccionado (entradas y efectos) teniendo en cuenta el filtro.
  - [ ] OK    Fix: 

- **`reloadWithoutScope`**
  - EN: Reload the selected snapshot (inputs and effects) without the scope.
  - ES: Recargar el snapshot seleccionado (entradas y efectos) sin el filtro.
  - [ ] OK    Fix: 
...
- **`snapshotSelector`**
  ...
  - ES: Seleccionar un snapshot sin cargarlo.
  - [ ] OK    Fix: 
...
- **`stereoAxisDial`**
  ...
  - ES: Gira el eje estéreo, positivo en sentido antihorario visto desde arriba. 0 = automático: el par se abre perpendicular a la línea del origen a la fuente. Sirve para orientar un par situado en el centro de un montaje circular. ±180° intercambia izquierda y derecha. También lo cambia un Space Mouse con Shift pulsado (giro) o un segundo dedo con Shift en el mapa (rotación). Con el Eje fijo activado, pasa a ser una orientación absoluta medida desde izquierda/derecha de sala. La rueda del ratón en el mapa también lo gira cuando el par está seleccionado.
  - [ ] OK    Fix: 
...
- **`stereoAxisLockButton`**
  ...
  - ES: Bloquea la orientación de la imagen estéreo: el par se abre a izquierda/derecha de sala girado solo por el valor de Eje, en lugar de seguir la línea del origen a la fuente. Útil cuando la imagen debe quedarse quieta mientras la fuente se mueve. L en el mapa lo conmuta para los pares seleccionados.
  - [ ] OK    Fix: 
...
- **`stereoWidthDial`**
  ...
  - ES: Anchura de la imagen estéreo en metros: la distancia física entre el brazo izquierdo y el derecho del par. La posición del canal sigue siendo el centro del par. Se comporta igual en arrays rectos, curvos, circulares y laterales. 0 m colapsa el par en una fuente puntual. También la cambia un Space Mouse con Shift pulsado (empujar/tirar) o un segundo dedo con Shift en el mapa (pellizco).
  - [ ] OK    Fix: 
...
- **`storeSnapshot`**
  ...
  - ES: Guardar un nuevo snapshot de todas las entradas y efectos.
  - [ ] OK    Fix: 
...
- **`updateSnapshot`**
  ...
  - ES: Actualizar el snapshot seleccionado, entradas y efectos (con copia de seguridad).
  - [ ] OK    Fix: 
...
- **`clusterEditAbsolute`**
  ...
  - ES: Edición con Ctrl+Shift: valor copiado a {count} otra(s) entrada(s) del Grupo {cluster}
  - [ ] OK    Fix: 

- **`clusterEditRelative`**
  - EN: Shift edit: applying relative change to {count} other input(s) of Cluster {cluster}
  - ES: Edición con Shift: cambio relativo aplicado a {count} otra(s) entrada(s) del Grupo {cluster}
  - [ ] OK    Fix: 
...
- **`midiBindingConflict`**
  ...
  - ES: La nota {note} del canal MIDI {ch} está asignada tanto a '{first}' como a '{other}' — prevalece '{first}'.
  - [ ] OK    Fix: 
...
- **`midiBindingNotSaved`**
  ...
  - ES: La nota MIDI no se guardó: OK solo conserva el alcance para el siguiente snapshot nuevo. Mantenga pulsado Update Snapshot Scope para guardar la nota.
  - [ ] OK    Fix: 

- **`midiPortConnected`**
  - EN: MIDI snapshot recall: '{name}' connected.
  - ES: Recuperar snapshots por MIDI: '{name}' conectado.
  - [ ] OK    Fix: 

- **`midiPortDisconnected`**
  - EN: MIDI snapshot recall: '{name}' is not connected. It reconnects when it comes back.
  - ES: Recuperar snapshots por MIDI: '{name}' no está conectado. Se reconectará cuando vuelva.
  - [ ] OK    Fix: 

- **`midiPortRefused`**
  - EN: MIDI snapshot recall: '{name}' could not be opened. Another application may be using it. Retrying every 2 s.
  - ES: Recuperar snapshots por MIDI: no se pudo abrir '{name}'. Puede que otra aplicación lo esté usando. Reintentando cada 2 s.
  - [ ] OK    Fix: 
...
- **`selectFolderFirst`**
  ...
  - ES: Por favor seleccione primero una carpeta de proyecto en System Config.
  - [ ] OK    Fix: 
...
- **`snapshotActionCancelled`**
  ...
  - ES: Se canceló la pulsación que mantenía sobre el botón de snapshot, porque cambió la selección.
  - [ ] OK    Fix: 
...
- **`snapshotEffectsSkipped`**
  ...
  - ES: Snapshot «{name}»: {n} entradas de efecto omitidas, no existe el canal de efecto {ids}
  - [ ] OK    Fix: 

- **`snapshotEntriesSkipped`**
  - EN: Snapshot '{name}' loaded; {n} entries skipped, no channel {numbers}
  - ES: Snapshot «{name}» cargado; {n} entradas omitidas, no existe el canal {numbers}
  - [ ] OK    Fix: 
...
- **`snapshotLoadedByMidi`**
  ...
  - ES: Snapshot '{name}' recuperado por MIDI.
  - [ ] OK    Fix: 
...
- **`snapshotNotFound`**
  ...
  - ES: Snapshot '{name}' no encontrado.
  - [ ] OK    Fix: 
...
- **`snapshotPatchMismatchApplied`**
  ...
  - ES: Snapshot «{name}» aplicado, pero se guardó con otro patch ({n} canal(es) difieren) — revise la lista de canales
  - [ ] OK    Fix: 

- **`snapshotRecallFailed`**
  - EN: Snapshot '{name}' could not be recalled: {error}
  - ES: No se pudo recuperar el snapshot '{name}': {error}
  - [ ] OK    Fix: 

- **`snapshotScopeUpdated`**
  - EN: Snapshot '{name}' scope updated.
  - ES: Alcance del snapshot '{name}' actualizado.
  - [ ] OK    Fix: 
...
- **`secondaryTouch`**
  ...
  - ES: Suspender o reactivar las ediciones con el segundo dedo: rotación y altura de las entradas, imagen estéreo, rotación y escala de los clusters. La panorámica y el zoom siguen funcionando. No se guarda.
  - [ ] OK    Fix: 
...
- **`trackingConflictsHeader`**
  ...
  - ES: Los siguientes clústeres tienen varias entradas con seguimiento activado:
  - [ ] OK    Fix: 
...
- **`admAxisSwap`**
  ...
  - ES: Qué eje ADM-OSC entrante se asigna a este eje interno.
  - [ ] OK    Fix: 

- **`admAzFlip`**
  - EN: Invert the direction of incoming azimuth.
  - ES: Invertir la dirección del azimut entrante.
  - [ ] OK    Fix: 

- **`admAzOffset`**
  - EN: Azimuth offset (deg) applied to incoming ADM-OSC azimuth.
  - ES: Offset de azimut (grados) aplicado al azimut ADM-OSC entrante.
  - [ ] OK    Fix: 

- **`admBreakpoint`**
  - EN: Normalized breakpoint (0-1) for piecewise linear stretch.
  - ES: Punto de quiebre normalizado (0-1) para estiramiento lineal por tramos.
  - [ ] OK    Fix: 

- **`admCenterOffset`**
  - EN: Physical position (m) where normalized 0.0 maps to.
  - ES: Posición física (m) donde el valor normalizado 0.0 se asigna.
  - [ ] OK    Fix: 

- **`admDistMax`**
  - EN: Maximum physical distance (m) at ADM-OSC distance=1.
  - ES: Distancia física máxima (m) en ADM-OSC distance=1.
  - [ ] OK    Fix: 

- **`admDistMin`**
  - EN: Minimum physical distance (m) at ADM-OSC distance=0.
  - ES: Distancia física mínima (m) en ADM-OSC distance=0.
  - [ ] OK    Fix: 

- **`admElFlip`**
  - EN: Invert the sign of incoming elevation.
  - ES: Invertir el signo de la elevación entrante.
  - [ ] OK    Fix: 

- **`admInnerWidth`**
  - EN: Physical extent (m) from center to breakpoint.
  - ES: Extensión física (m) del centro al punto de quiebre.
  - [ ] OK    Fix: 

- **`admInputAssign`**
  - EN: Assign this input to an ADM-OSC mapping for receive/transmit.
  - ES: Asignar esta entrada a una asignación ADM-OSC para recepción/transmisión.
  - [ ] OK    Fix: 

- **`admLinkAll`**
  - EN: Select all 6 sides at once for uniform editing.
  - ES: Seleccionar los 6 lados a la vez para edición uniforme.
  - [ ] OK    Fix: 
...
- **`admOuterWidth`**
  ...
  - ES: Extensión física (m) del punto de quiebre a ±1.
  - [ ] OK    Fix: 

- **`admSideSelect`**
  - EN: Select sides to edit. Changes apply to all selected sides at once.
  - ES: Seleccionar lados a editar. Los cambios se aplican a todos los lados seleccionados simultáneamente.
  - [ ] OK    Fix: 

- **`admSignFlip`**
  - EN: Invert the sign of the incoming axis value.
  - ES: Invertir el signo del valor del eje entrante.
  - [ ] OK    Fix: 
...
- **`networkInterface`**
  ...
  - ES: Seleccionar la Network Interface.
  - [ ] OK    Fix: 
...
- **`trackingOscPath`**
  ...
  - ES: OSC Path para el tracking en modo OSC (comienza con /)
  - [ ] OK    Fix: 
...
- **`notResponding`**
  ...
  - ES: Remote no responde — compruebe la IP/puerto de la tableta y que WFS Control esté abierto y actualizado
  - [ ] OK    Fix: 
...
- **`sendFailing`**
  ...
  - ES: El envío a la tableta falla — en macOS, permita WFS-DIY en Ajustes del Sistema › Privacidad y seguridad › Red local, y compruebe que este ordenador está en la red de la tableta
  - [ ] OK    Fix: 
...
- **`arrayMute`**
  ...
  - ES: Silenciar o reactivar todos los altavoces del grupo de esta salida. Solo para la sesión: no se guarda con el proyecto y se anula al cargar uno. No disponible para salidas Single.
  - [ ] OK    Fix: 
...
- **`eqFreq`**
  ...
  - ES: Frecuencia Output EQ banda {band} (20 Hz - 20 kHz).
  - [ ] OK    Fix: 

- **`eqGain`**
  - EN: Output EQ Band {band} gain (-24 to +24 dB).
  - ES: Ganancia Output EQ banda {band} (-24 a +24 dB).
  - [ ] OK    Fix: 

- **`eqQ`**
  - EN: Output EQ Band {band} Q factor (0.1 - 10.0).
  - ES: Factor Q Output EQ banda {band} (0.1 - 10.0).
  - [ ] OK    Fix: 
...
- **`eqShape`**
  ...
  - ES: Forma del filtro Output EQ banda {band}.
  - [ ] OK    Fix: 
...
- **`floorReflections`**
  ...
  - ES: Permitir o excluir este altavoz de las Floor Reflections.
  - [ ] OK    Fix: 
...
- **`liveSource`**
  ...
  - ES: Permitir o excluir este altavoz de la atenuación Live Source. (puede afectar al resto del grupo)
  - [ ] OK    Fix: 
...
- **`minLatency`**
  ...
  - ES: Permitir o excluir este altavoz del procesamiento Minimal Latency. (puede afectar al resto del grupo)
  - [ ] OK    Fix: 
...
- **`arrayEditSingle`**
  ...
  - ES: Edición con Ctrl: cambio aplicado solo a esta salida (Grupo {array} no afectado)
  - [ ] OK    Fix: 

- **`arrayMuted`**
  - EN: Array {array} muted
  - ES: Grupo {array} silenciado
  - [ ] OK    Fix: 

- **`arrayUnmuted`**
  - EN: Array {array} unmuted
  - ES: Grupo {array} reactivado
  - [ ] OK    Fix: 
...
- **`configNotFound`**
  ...
  - ES: Archivo de configuración de salidas no encontrado.
  - [ ] OK    Fix: 

- **`configReloaded`**
  - EN: Output configuration reloaded.
  - ES: Configuración de salidas recargada.
  - [ ] OK    Fix: 
...
- **`noBackup`**
  ...
  - ES: No se encontró respaldo de configuración de salidas.
  - [ ] OK    Fix: 
...
- **`algoFDNGpu`**
  ...
  - ES: Ejecuta la reverb FDN en la GPU (añade ~20 ms solo a la señal wet) o en la CPU. Vuelve automáticamente a la CPU si la GPU no está disponible.
  - [ ] OK    Fix: 
...
- **`algoIRGpu`**
  ...
  - ES: Ejecuta la convolución IR en la GPU (añade ~20 ms solo a la señal wet) o en la CPU. Vuelve automáticamente a la CPU si la GPU no está disponible.
  - [ ] OK    Fix: 
...
- **`algoSDNGpu`**
  ...
  - ES: Ejecuta la reverb SDN en la GPU (añade ~20 ms solo a la señal wet) o en la CPU. Vuelve automáticamente a la CPU si la GPU no está disponible.
  - [ ] OK    Fix: 
...
- **`applyToAllNodes`**
  ...
  - ES: Cuando está activado, los cambios de parámetros se aplican a todos los nodos de reverberación a la vez; cuando está desactivado, solo al nodo seleccionado. Solo en tiempo de ejecución - solo se propagan los cambios futuros.
  - [ ] OK    Fix: 
...
- **`distanceAtten`**
  ...
  - ES: Distance attenuation para el Reverb Return (-6.0 a 0.0 dB/m).
  - [ ] OK    Fix: 

- **`distanceAttenEnable`**
  - EN: Distance attenuation percentage (0-200%).
  - ES: Porcentaje de Distance attenuation (0-200%).
  - [ ] OK    Fix: 
...
- **`mapVisibility`**
  ...
  - ES: Hacer visibles u ocultar todos los Reverb Channels en el mapa.
  - [ ] OK    Fix: 

- **`miniLatencyTooltip`**
  - EN: Allow or exclude this reverb feed from Minimal Latency processing.
  - ES: Permitir o excluir este envío de reverberación del procesamiento Minimal Latency.
  - [ ] OK    Fix: 
...
- **`conflict`**
  ...
  - ES: En uso por '{name}' (canal {ch}, nota {note}) — elija otra nota para guardar.
  - [ ] OK    Fix: 
...
- **`deleteMessage`**
  ...
  - ES: ¿Eliminar el canal de entrada {number}? Sus ajustes se descartan. En una sesión nueva los canales restantes se renumeran siguiendo el orden de visualización; en cuanto la sesión está en uso, su número queda retirado como hueco y los demás conservan los suyos, así que el patch, los snapshots, los cues de QLab y las asignaciones DAW siguen siendo válidos.
  - [ ] OK    Fix: 

- **`deleteTitle`**
  - EN: Remove Input Channel
  - ES: Eliminar canal de entrada
  - [ ] OK    Fix: 

- **`dragHint`**
  - EN: Drag a channel to change the order.
  - ES: Arrastra un canal para cambiar el orden.
  - [ ] OK    Fix: 
...
- **`dragHintFresh`**
  ...
  - ES: Sesión nueva: este orden se convierte en la numeración de canales y el patch se dispone a juego. Coloque ahora los canales estéreo y mono, antes de guardar o patchear nada.
  - [ ] OK    Fix: 
...
- **`dragHintInUse`**
  ...
  - ES: Sesión en uso: los números son permanentes. Reordenar solo cambia lo que se ve — cada canal conserva sus entradas de hardware, y los snapshots, cues y remotos siguen apuntando al mismo canal. La matriz de patch dejará de leerse como una diagonal.
  - [ ] OK    Fix: 
...
- **`fromFileHelp`**
  ...
  - ES: Adoptar los números o el orden de canales de una config de sistema o de entradas guardada, sin cargar nada más.
  - [ ] OK    Fix: 

- **`fromFileTitle`**
  - EN: Take the channel list from a saved config
  - ES: Tomar la lista de canales de una config guardada
  - [ ] OK    Fix: 
...
- **`title`**
  ...
  - ES: Organizar canales de entrada
  - [ ] OK    Fix: 
...
- **`channelIdentity.alreadyMatches`**
  ...
  - ES: La lista de canales ya coincide con {file}.
  - [ ] OK    Fix: 

- **`channelIdentity.cancelled`**
  - EN: Load cancelled: channel list not reconciled.
  - ES: Carga cancelada: lista de canales sin reconciliar.
  - [ ] OK    Fix: 
...
- **`channelIdentity.conflictOnly`**
  ...
  - ES: Las listas están en conflicto, no se puede adoptar nada con seguridad. Cargar este archivo haría lo siguiente:
  - [ ] OK    Fix: 

- **`channelIdentity.fixFailed`**
  - EN: Could not reconcile the channel list: {error}
  - ES: No se pudo reconciliar la lista de canales: {error}
  - [ ] OK    Fix: 

- **`channelIdentity.fixed`**
  - EN: Channel list now matches {file}.
  - ES: La lista de canales ahora coincide con {file}.
  - [ ] OK    Fix: 

- **`channelIdentity.intro`**
  - EN: {file} describes a different channel list than this session.
  - ES: {file} describe una lista de canales distinta de esta sesión.
  - [ ] OK    Fix: 

- **`channelIdentity.lineAdded`**
  - EN:   #{number} ({type}) is created with default settings
  - ES:   #{number} ({type}) se crea con ajustes por defecto
  - [ ] OK    Fix: 
...
- **`channelIdentity.linePatch`**
  ...
  - ES:   {channel}: guardado en la entrada {file}, ahora en {live}
  - [ ] OK    Fix: 

- **`channelIdentity.lineRemoved`**
  - EN:   {channel} is removed with its settings
  - ES:   {channel} se elimina con sus ajustes
  - [ ] OK    Fix: 

- **`channelIdentity.lineRetyped`**
  - EN:   {channel} becomes {newType} and keeps its current settings
  - ES:   {channel} pasa a ser {newType} y conserva sus ajustes actuales
  - [ ] OK    Fix: 
...
- **`channelIdentity.lineRowsCross`**
  ...
  - ES:   las entradas de hardware se quedan en su posición de fila, así que cada canal movido recibe las entradas de otro
  - [ ] OK    Fix: 

- **`channelIdentity.loadAnyway`**
  - EN: Load anyway applies the file's list by channel NUMBER:
  - ES: Cargar de todos modos aplica la lista del archivo por NÚMERO de canal:
  - [ ] OK    Fix: 

- **`channelIdentity.loadAnywayButton`**
  - EN: Load anyway
  - ES: Cargar de todos modos
  - [ ] OK    Fix: 
...
- **`channelIdentity.noIdentity`**
  ...
  - ES: {file} fue guardado por una versión anterior y solo contiene un número de canales ({count}); esta sesión tiene {liveCount}. Se conserva la lista actual y el patch del archivo se aplica por posición de fila; {delta}.
  - [ ] OK    Fix: 

- **`channelIdentity.noIdentityAppend`**
  - EN: {n} mono channel(s) are appended
  - ES: se añaden {n} canales mono
  - [ ] OK    Fix: 

- **`channelIdentity.noIdentityKeep`**
  - EN: no channel is added or removed
  - ES: no se añade ni elimina ningún canal
  - [ ] OK    Fix: 

- **`channelIdentity.noIdentityRemove`**
  - EN: the {n} highest-numbered channel(s) are removed: {channels}
  - ES: se eliminan los {n} canales de número más alto: {channels}
  - [ ] OK    Fix: 

- **`channelIdentity.noIdentityShort`**
  - EN: {file} carries no channel list to adopt (saved by an older version).
  - ES: {file} no contiene una lista de canales que adoptar (versión anterior).
  - [ ] OK    Fix: 

- **`channelIdentity.notAConfig`**
  - EN: {file} is not a system or input config file.
  - ES: {file} no es un archivo de config de sistema o de entradas.
  - [ ] OK    Fix: 

- **`channelIdentity.notUndoable`**
  - EN: None of this can be undone, and it clears the undo history of every tab.
  - ES: Nada de esto se puede deshacer, y se borra el historial de deshacer de todas las pestañas.
  - [ ] OK    Fix: 
...
- **`channelIdentity.pairAdvice`**
  ...
  - ES: Esto ocurre cuando uno de los dos se guardó por separado. Cancele y guarde una vez la configuración completa para volver a sincronizarlos.
  - [ ] OK    Fix: 

- **`channelIdentity.pairLine`**
  - EN:   {channel} becomes #{newNumber}
  - ES:   {channel} pasa a ser #{newNumber}
  - [ ] OK    Fix: 
...
- **`channelIdentity.pairMismatch`**
  ...
  - ES: system.xml e inputs.xml de este proyecto no coinciden en la lista de canales. system.xml decide números, tipos y patch; inputs.xml decide después ajustes, tipos y orden:
  - [ ] OK    Fix: 

- **`channelIdentity.patchOnly`**
  - EN: Same channel list, but it was saved under a different patching:
  - ES: Misma lista de canales, pero guardada con otro patch:
  - [ ] OK    Fix: 
...
- **`channelIdentity.relabelOffer`**
  ...
  - ES: La disposición coincide posición por posición — los mismos canales mono y estéreo en los mismos lugares — pero los números difieren.\n\nTomar los números del archivo: cada canal conserva su lugar, sus ajustes y sus entradas de hardware, y recibe el número del archivo para esa posición:
  - [ ] OK    Fix: 
...
- **`channelIdentity.relabelOfferHw`**
  ...
  - ES: Por sus entradas de hardware, estos canales parecen ser los mismos con otros números.\n\nTomar los números del archivo: cada canal conserva su lugar, sus ajustes y sus entradas de hardware; solo cambia su número, para que los snapshots, cues y OSC que usan los números del archivo lleguen a los canales correctos:
  - [ ] OK    Fix: 
...
- **`channelIdentity.reorderOffer`**
  ...
  - ES: Los mismos canales en distinto orden.\n\nReordenar primero: los canales se colocan en el orden del archivo y conservan sus números, ajustes y entradas de hardware.
  - [ ] OK    Fix: 

- **`channelIdentity.stillDiffers`**
  - EN: The channel list still differs from {file} after reconciling; load cancelled.
  - ES: La lista de canales sigue difiriendo de {file} tras reconciliar; carga cancelada.
  - [ ] OK    Fix: 

- **`channelIdentity.takeNumbers`**
  - EN: Take the file's numbers
  - ES: Tomar los números del archivo
  - [ ] OK    Fix: 

- **`channelIdentity.takeNumbersHw`**
  - EN: Take the file's numbers (by hardware input)
  - ES: Tomar los números del archivo (por entrada de hardware)
  - [ ] OK    Fix: 

- **`channelIdentity.title`**
  - EN: Channel list differs from the file
  - ES: La lista de canales difiere del archivo
  - [ ] OK    Fix: 
...
- **`dialsAndButtons`**
  ...
  - ES: Seleccionar el controlador hardware para diales y botones: Stream Deck+.
  - [ ] OK    Fix: 
...
- **`editChannels`**
  ...
  - ES: Organizar los canales de entrada: arrastra para intercalar mono y estéreo en cualquier orden, o elimina un canal concreto. En una sesión nueva los números siguen el orden de visualización; se vuelven permanentes en cuanto la sesión está en uso — un proyecto cargado, la pestaña Inputs o Map abierta, la ventana de patch abierta, un snapshot guardado o un control externo conectado. A partir de ahí un canal conserva su número allá donde lo arrastres y un número eliminado queda retirado como hueco — el patch, los snapshots, los cues de QLab y las asignaciones del plug-in DAW nunca se rompen. Bloqueado mientras el DSP está en marcha.
  - [ ] OK    Fix: 
...
- **`gpuPipelineDepth`**
  ...
  - ES: Profundidad del pipeline GPU en bloques de audio. Añade depth x búfer/frecuencia de muestreo de latencia constante (pre-restada de los retardos WFS) y absorbe pausas de la GPU de la misma duración: más profundo = inmune a tirones del escritorio/IU, menos profundo = menor latencia. Se aplica en directo. Predeterminado 4.
  - [ ] OK    Fix: 
...
- **`inputChannels`**
  ...
  - ES: Número de canales de entrada mono. Aumentar añade canales después del último; reducir elimina los canales mono con los números más altos. Bloqueado mientras el DSP está en marcha.
  - [ ] OK    Fix: 
...
- **`masterLevel`**
  ...
  - ES: Master Level (afecta a todas las salidas).
  - [ ] OK    Fix: 
...
- **`positionControl`**
  ...
  - ES: Seleccionar el controlador hardware para el control de posición: Space Mouse, Joystick o gamepad. Con un Space Mouse, mantener Shift en una entrada estéreo convierte el giro en el eje estéreo y empujar/tirar en la anchura estéreo.
  - [ ] OK    Fix: 
...
- **`speedOfSound`**
  ...
  - ES: Speed of Sound (relacionada con la temperatura).
  - [ ] OK    Fix: 
...
- **`stereoInputChannels`**
  ...
  - ES: Número de entradas estéreo (pares). Cada par conserva un número de canal y ocupa dos entradas de hardware (L y R) en el patch. Aumentar añade después del último canal; reducir elimina los estéreo con los números más altos. Bloqueado mientras el DSP está en marcha.
  - [ ] OK    Fix: 
