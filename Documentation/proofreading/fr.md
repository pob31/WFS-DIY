...
- **`midiTooltip`**
  ...
  - FR: Rappel de snapshots uniquement. Ce port est ouvert en entrée seule et ne sert à rien d'autre : WFS-DIY ne lui envoie aucune donnée MIDI, et les seuls messages pris en compte sont les note-on qui rappellent un snapshot (entrées et effets). Affectez un canal et une note à un snapshot dans la fenêtre Edit Scope de l'onglet Inputs ou Effects ; un note-on de vélocité supérieure à 64 le rappelle.
  - [ ] OK    Fix: 
...
- **`unpatchInputsMessage`**
  ...
  - FR: Êtes-vous sûr de vouloir supprimer tous les routages d'entrée ?
  - [ ] OK    Fix: 

- **`unpatchInputsTitle`**
  - EN: Unpatch All Inputs
  - FR: Déconnecter toutes les entrées
  - [ ] OK    Fix: 

- **`unpatchOutputsMessage`**
  - EN: Are you sure you want to remove all output patches?
  - FR: Êtes-vous sûr de vouloir supprimer tous les routages de sortie ?
  - [ ] OK    Fix: 

- **`unpatchOutputsTitle`**
  - EN: Unpatch All Outputs
  - FR: Déconnecter toutes les sorties
  - [ ] OK    Fix: 
...
- **`channelNotAvailable`**
  ...
  - FR: Le canal {channel} n'est pas ouvert sur le périphérique audio actuel
  - [ ] OK    Fix: 
...
- **`referenceMode`**
  ...
  - FR: Sélectionner le point de référence pour les transformations du cluster : First Input, Barycentre ou Shared Position (tous les membres coïncident ; échelle et rotation s'appliquent aux décalages individuels).
  - [ ] OK    Fix: 
...
- **`exportTitle`**
  ...
  - FR: Exporter configuration effets
  - [ ] OK    Fix: 

- **`importTitle`**
  - EN: Import Effects Configuration
  - FR: Importer configuration effets
  - [ ] OK    Fix: 
...
- **`editOnMap`**
  ...
  - FR: Activer la modification des positions des effets depuis l'onglet Map.
  - [ ] OK    Fix: 
...
- **`groupMute`**
  ...
  - FR: Appui long pour couper ou rétablir d'un coup le son de tous les membres de ce groupe de liaison.
  - [ ] OK    Fix: 
...
- **`linkGroup`**
  ...
  - FR: Lier cet effet à un groupe pour que les modifications des modules se propagent aux autres membres.
  - [ ] OK    Fix: 

- **`linkMode`**
  - EN: How this channel takes group edits: off, absolute, or relative.
  - FR: Comment ce canal reçoit les modifications du groupe : désactivé, absolu ou relatif.
  - [ ] OK    Fix: 

- **`loopGuard`**
  - EN: The loop guard has muted this channel's effect-to-effect feed.
  - FR: La garde de boucle a coupé l'alimentation d'effet à effet de ce canal.
  - [ ] OK    Fix: 
...
- **`solo`**
  ...
  - FR: Appui long pour mettre ce retour d'effet en solo.
  - [ ] OK    Fix: 

- **`soloEffects`**
  - EN: Long-press to solo the effect returns: the direct signal is muted.
  - FR: Appui long pour mettre les retours d'effets en solo : le signal direct est coupé.
  - [ ] OK    Fix: 
...
- **`cleared`**
  ...
  - FR: Chaîne d'effets vidée.
  - [ ] OK    Fix: 

- **`clearedAll`**
  - EN: Every effect chain cleared.
  - FR: Toutes les chaînes d'effets vidées.
  - [ ] OK    Fix: 

- **`configLoaded`**
  - EN: Effects configuration loaded.
  - FR: Configuration des effets chargée.
  - [ ] OK    Fix: 

- **`configSaved`**
  - EN: Effects configuration saved.
  - FR: Configuration des effets enregistrée.
  - [ ] OK    Fix: 
...
- **`body`**
  ...
  - FR: Les outils de diagnostic sont masqués par défaut : un appui long sur le bouton Diagnostic ci-dessous les affiche ou les masque. Ils apparaissent automatiquement si la session précédente ne s'est pas terminée proprement.\nPour envoyer un retour ou signaler un problème, cliquez sur « Signaler un problème » (Report Issue) : le suivi des tickets GitHub du projet s'ouvre dans votre navigateur. Décrivez ce qui s'est passé, ce que vous attendiez et les étapes pour reproduire le problème, puis joignez les données de diagnostic exportées.\n« Exporter les journaux » (Export Logs) copie les données utiles dans un dossier WFS-DIY-logs à l'emplacement de votre choix : les journaux de la session en cours et jusqu'à cinq sessions précédentes, plus le fichier de réglages de l'application. Joignez ce dossier (ou un zip de celui-ci) à votre rapport.\nLes journaux de session contiennent les informations de démarrage (version de l'application, système d'exploitation, CPU, nombre de canaux), le chargement des projets, l'activité réseau et les erreurs. Aucun audio n'est jamais enregistré.\n« Ouvrir le dossier des journaux » (Open Log Folder) affiche les journaux bruts sur le disque (dossier WFS-DIY/logs des données d'application de l'utilisateur), utile pour retrouver une session précise.\n« Copier les infos système » (Copy System Info) place un court résumé dans le presse-papiers — version de l'application, système d'exploitation, CPU et périphérique audio actuel avec sa fréquence d'échantillonnage et sa taille de tampon — prêt à coller dans un ticket.
  - [ ] OK    Fix: 

- **`title`**
  - EN: Diagnostics & Feedback
  - FR: Diagnostic et retours
  - [ ] OK    Fix: 
...
- **`body`**
  ...
  - FR: Un canal d'effet est une chaîne de onze modules (distorsion, deux EQ, deux dynamiques, chorus/flanger, phaser, trémolo, réverbération, délai multi-taps, bitcrusher) alimentée par les entrées et par d'autres effets via la matrice d'envois. Sa sortie est un RETOUR placé sur scène comme une entrée : il est rendu par les réseaux WFS depuis sa propre position, avec la même orientation, la même loi d'atténuation et les mêmes mutes qu'une entrée.\nL'en-tête reprend celui de l'onglet Reverb : sélecteur de canal, nom, visibilité sur la carte, Edit on Map, Solo Effects, Mute, Solo et Clear d'urgence (appui long ; maintenir Ctrl pour vider toutes les chaînes). Les trois voyants donnent l'état du moteur pour ce canal : garde de boucle déclenchée, membre d'une boucle de réinjection, alimenté par une entrée (point d'entrée de son groupe d'effets).\nLes groupes de liaison ne sont pas comme les réverbs, qui forment un seul système : un canal d'un groupe partage sa chaîne (ordre, bypass, paramètres des modules) avec les autres membres selon le mode de liaison de chacun — Absolute copie, Relative conserve l'écart, Off ne reçoit rien — tandis que ses mouvements, envois, mutes et position ne se propagent jamais. Ctrl-glisser un contrôle pour ne modifier que ce canal. Mute Group coupe chaque membre une fois et laisse chacun modifiable séparément.
  - [ ] OK    Fix: 
...
- **`body`**
  ...
  - FR: Réflexions du Sol\n\nSimuler les réflexions du sol améliore le naturel du son. On ne s'attend pas à ce que les sons soient diffusés dans une chambre anéchoïque insonorisée. Ce réglage aide à recréer les réflexions du sol attendues.\nLe niveau des réflexions du sol peut être ajusté ainsi que les filtres coupe-bas et le shelf hautes fréquences. La diffusion ajoute un peu de chaos pour simuler les irrégularités du sol.
  - [ ] OK    Fix: 

- **`title`**
  - EN: Floor Reflections
  - FR: Floor Reflections
  - [ ] OK    Fix: 
...
- **`legendMaxAttenuation`**
  ...
  - FR: atténuation maximale
  - [ ] OK    Fix: 

- **`legendNoAttenuation`**
  - EN: no attenuation
  - FR: pas d'atténuation
  - [ ] OK    Fix: 

- **`legendPosition`**
  - EN: position of the source
  - FR: position de la source
  - [ ] OK    Fix: 
...
- **`body`**
  ...
  - FR: - Un clic gauche sur une entrée ou un cluster permet de le déplacer en glissant. Un toucher simple fait de même.\n- Un clic gauche avec Maj ajoute ou retire des entrées de la sélection. Un double tap et glisser fait de même.\n- Un clic gauche glissé dessine un rectangle de sélection.\n- Un double-clic ou tap réinitialise le décalage de position.\n- Un clic gauche long ou un appui long (0,7 à 1,2 s) sans mouvement ouvre, au relâchement, l'onglet des entrées sur l'entrée sélectionnée. S'éloigner l'annule, même si l'on revient, et le geste est ignoré pendant 3 s après le déplacement ou la modification d'un marqueur.\n- Un clic hors d'une entrée efface la sélection.\n- Un clic droit glissé déplace la vue. Un glisser à deux doigts fait de même si le multitouch est supporté.\n- La molette zoome. Un pincement à deux doigts aussi.\n- Lorsque des entrées stéréo sont sélectionnées, la molette tourne leur axe d'image au lieu de zoomer : 5° par cran, 1° avec Maj. Maintenir Ctrl/Cmd pour zoomer comme d'habitude.\n- Un clic molette réinitialise la vue.\n- Les flèches déplacent la sélection en X/Y, PageHaut/Bas en hauteur.\n- Un deuxième doigt près d'une entrée touchée peut tourner la directivité et ajuster la hauteur.\n- Un deuxième doigt près d'un cluster touché peut le tourner et le redimensionner.\n- Sur une entrée stéréo touchée, avec la touche Maj enfoncée le deuxième doigt agit sur l'image stéréo : la rotation change l'axe de l'image et le pincement change la largeur.\n- Le bouton situé sous celui qui permet d'afficher les niveaux audio suspend toutes les modifications au deuxième doigt sur les entrées et les clusters. La vue peut toujours être déplacée et zoomée, et les paires de nœuds de réverbération déplacées en symétrie. Ce réglage n'est pas enregistré.\n- Avec une Space Mouse, en maintenant Maj avec une entrée stéréo sélectionnée, la torsion change l'axe de l'image et tirer/pousser change la largeur, dans le même sens que le deuxième doigt : tirer vers le haut élargit, pousser vers le bas resserre. Maj avec XY déplace toujours l'entrée.\n- Appuyer sur L avec des entrées stéréo sélectionnées verrouille ou déverrouille l'orientation de leur image. Verrouillée, la paire s'ouvre à gauche/droite de la salle, tournée par la seule valeur d'Axe, et ne suit plus la source autour de l'origine.\n- Les entrées, tableaux de sorties et nœuds de réverbération peuvent être masqués.\n- Les entrées peuvent être verrouillées pour empêcher leur sélection et déplacement.\n- Les nœuds de réverbération peuvent être déplacés si activé. Ctrl/Cmd déplace les paires en symétrie.\n- Le rayon du Live Source Tamer est affiché autour de l'entrée quand activé.\n- Un bouton permet d'afficher les niveaux audio sur la carte.
  - [ ] OK    Fix: 
...
- **`body`**
  ...
  - FR: Les snapshots stockent les paramètres d'entrée et d'effet, mais peuvent avoir une portée pour être rappelés pendant un spectacle. Ils peuvent contenir de tous les paramètres pour toutes les entrées et tous les effets à un seul paramètre pour un seul canal.\nLa Portée indique au système quelles données stocker ou rappeler. C'est l'opposé des paramètres 'sûrs'.\nPlusieurs méthodes sont disponibles :\n- Enregistrer uniquement les données nécessaires dans des fichiers locaux. Le filtre de portée est appliqué lors du stockage.\n- Enregistrer toutes les données et un filtre dans des fichiers locaux. Le filtre est appliqué au rappel, permettant éventuellement de tout rappeler.\n- Enregistrer toutes les données dans des cues QLab. Non recommandé pour les grandes configurations.\nLa portée peut afficher et présélectionner automatiquement les paramètres modifiés manuellement. Les paramètres modifiés sont marqués en jaune.\nUn snapshot contient les entrées et les canaux d'effet. La fenêtre de portée a un onglet pour les entrées et un pour les effets, une grille par famille ; dans la grille des effets, chaque module de la chaîne est une ligne, bandes et taps compris. Les onglets Entrées et Effets affichent la même rangée de snapshots : un snapshot enregistré ou rappelé depuis l'un ou l'autre couvre les deux, et une note MIDI ou un cue OSC rappelle les deux.
  - [ ] OK    Fix: 

- **`title`**
  - EN: Snapshots and Scope
  - FR: Snapshots et Portée
  - [ ] OK    Fix: 
...
- **`snapshotIdentity.fixNumbers`**
  ...
  - FR: Corriger les numéros d'abord
  - [ ] OK    Fix: 

- **`snapshotIdentity.intro`**
  - EN: Snapshot '{name}' was stored when these channels were on other hardware inputs:
  - FR: Le snapshot « {name} » a été enregistré quand ces canaux étaient sur d'autres entrées matérielles :
  - [ ] OK    Fix: 
...
- **`snapshotIdentity.noFix`**
  ...
  - FR: Aucune correspondance matérielle un-à-un n'a été trouvée, aucune renumérotation automatique n'est proposée. Si le câblage a été modifié volontairement, continuez ; sinon annulez et vérifiez la liste des canaux.
  - [ ] OK    Fix: 
...
- **`snapshotIdentity.proceedNote`**
  ...
  - FR: Continuer applique le snapshot par numéro de canal tel quel. Annuler ne change rien.
  - [ ] OK    Fix: 

- **`snapshotIdentity.title`**
  - EN: Snapshot stored under a different patching
  - FR: Snapshot enregistré avec un autre patch
  - [ ] OK    Fix: 
...
- **`snapshotOverwriteWarning`**
  ...
  - FR: Un snapshot portant ce nom existe déjà — il sera écrasé.
  - [ ] OK    Fix: 
...
- **`attenuationLawButton`**
  ...
  - FR: Modèle d'Attenuation Law (décroissance linéaire du volume avec la distance entre l'objet et l'enceinte, ou quadratique).
  - [ ] OK    Fix: 
...
- **`deleteSnapshot`**
  ...
  - FR: Supprimer le snapshot sélectionné avec confirmation.
  - [ ] OK    Fix: 
...
- **`lsPeakEnableButton`**
  ...
  - FR: Activer ou désactiver le compresseur rapide (crête) pour le Live Source Tamer.
  - [ ] OK    Fix: 
...
- **`lsSlowEnableButton`**
  ...
  - FR: Activer ou désactiver le compresseur lent pour le Live Source Tamer.
  - [ ] OK    Fix: 
...
- **`minimalLatencyButton`**
  ...
  - FR: Choisir entre Acoustic Precedence et Minimal Latency pour la précédence d'amplification.
  - [ ] OK    Fix: 
...
- **`muteMacrosSelector`**
  ...
  - FR: Mute Macros pour couper et réactiver rapidement les arrays.
  - [ ] OK    Fix: 
...
- **`reloadSnapshot`**
  ...
  - FR: Recharger le snapshot sélectionné (entrées et effets) en tenant compte du filtre.
  - [ ] OK    Fix: 

- **`reloadWithoutScope`**
  - EN: Reload the selected snapshot (inputs and effects) without the scope.
  - FR: Recharger le snapshot sélectionné (entrées et effets) sans le filtre.
  - [ ] OK    Fix: 
...
- **`snapshotSelector`**
  ...
  - FR: Sélectionner un snapshot sans le charger.
  - [ ] OK    Fix: 

- **`solo`**
  - EN: Listen to Binaural Rendering of this channel.
  - FR: Écouter le Binaural Renderer de ce canal.
  - [ ] OK    Fix: 
...
- **`stereoAxisDial`**
  ...
  - FR: Fait pivoter l'axe stéréo, positif dans le sens antihoraire vu de dessus. 0 = automatique : la paire s'étale perpendiculairement à la ligne allant de l'origine à la source. Permet d'orienter une paire placée au centre d'un dispositif circulaire. ±180° échange la gauche et la droite. Une Space Mouse avec Maj enfoncée (torsion) ou un deuxième doigt avec Maj sur la carte (rotation) le modifie aussi. Avec l'Axe fixe activé, il devient une orientation absolue mesurée depuis gauche/droite de la salle. La molette sur la carte le tourne également lorsque la paire est sélectionnée.
  - [ ] OK    Fix: 
...
- **`stereoAxisLockButton`**
  ...
  - FR: Verrouille l'orientation de l'image stéréo : la paire s'ouvre à gauche/droite de la salle, tournée par la seule valeur d'Axe, au lieu de suivre la ligne de l'origine à la source. Utile lorsque l'image doit rester fixe pendant que la source se déplace. L sur la carte l'active ou la désactive pour les paires sélectionnées.
  - [ ] OK    Fix: 
...
- **`stereoWidthDial`**
  ...
  - FR: Largeur de l'image stéréo en mètres : la distance physique entre les branches gauche et droite de la paire. La position du canal reste le centre de la paire. Comportement identique sur les réseaux droits, courbes, circulaires et latéraux. 0 m réduit la paire à une source ponctuelle. Une Space Mouse avec Maj enfoncée (pousser/tirer) ou un deuxième doigt avec Maj sur la carte (pincement) la modifie aussi.
  - [ ] OK    Fix: 
...
- **`storeSnapshot`**
  ...
  - FR: Enregistrer un nouveau snapshot de toutes les entrées et de tous les effets.
  - [ ] OK    Fix: 
...
- **`updateSnapshot`**
  ...
  - FR: Mettre à jour le snapshot sélectionné, entrées et effets (avec sauvegarde).
  - [ ] OK    Fix: 
...
- **`clusterEditAbsolute`**
  ...
  - FR: Édition Ctrl+Maj : valeur copiée sur {count} autre(s) entrée(s) du Groupe {cluster}
  - [ ] OK    Fix: 

- **`clusterEditRelative`**
  - EN: Shift edit: applying relative change to {count} other input(s) of Cluster {cluster}
  - FR: Édition Maj : changement relatif appliqué à {count} autre(s) entrée(s) du Groupe {cluster}
  - [ ] OK    Fix: 
...
- **`midiBindingConflict`**
  ...
  - FR: Le canal MIDI {ch}, note {note}, est affecté à la fois à « {first} » et à « {other} » — « {first} » l'emporte.
  - [ ] OK    Fix: 
...
- **`midiBindingNotSaved`**
  ...
  - FR: La note MIDI n'a pas été enregistrée : OK ne conserve la portée que pour le prochain nouveau snapshot. Appuyez longuement sur Update Snapshot Scope pour enregistrer la note.
  - [ ] OK    Fix: 

- **`midiPortConnected`**
  - EN: MIDI snapshot recall: '{name}' connected.
  - FR: Rappel MIDI des snapshots : « {name} » connecté.
  - [ ] OK    Fix: 

- **`midiPortDisconnected`**
  - EN: MIDI snapshot recall: '{name}' is not connected. It reconnects when it comes back.
  - FR: Rappel MIDI des snapshots : « {name} » n'est pas connecté. Il se reconnectera dès son retour.
  - [ ] OK    Fix: 

- **`midiPortRefused`**
  - EN: MIDI snapshot recall: '{name}' could not be opened. Another application may be using it. Retrying every 2 s.
  - FR: Rappel MIDI des snapshots : impossible d'ouvrir « {name} ». Une autre application l'utilise peut-être. Nouvelle tentative toutes les 2 s.
  - [ ] OK    Fix: 
...
- **`selectFolderFirst`**
  ...
  - FR: Veuillez d'abord sélectionner un dossier projet dans System Config.
  - [ ] OK    Fix: 
...
- **`snapshotActionCancelled`**
  ...
  - FR: Votre appui long sur le bouton de snapshot a été annulé, car la sélection a changé.
  - [ ] OK    Fix: 
...
- **`snapshotEffectsSkipped`**
  ...
  - FR: Snapshot « {name} » : {n} entrées d'effet ignorées, pas de canal d'effet {ids}
  - [ ] OK    Fix: 

- **`snapshotEntriesSkipped`**
  - EN: Snapshot '{name}' loaded; {n} entries skipped, no channel {numbers}
  - FR: Snapshot « {name} » chargé ; {n} entrées ignorées, pas de canal {numbers}
  - [ ] OK    Fix: 
...
- **`snapshotLoadedByMidi`**
  ...
  - FR: Snapshot « {name} » rappelé par MIDI.
  - [ ] OK    Fix: 
...
- **`snapshotPatchMismatchApplied`**
  ...
  - FR: Snapshot « {name} » appliqué, mais il a été enregistré avec un autre patch ({n} canal(aux) diffèrent) — vérifiez la liste des canaux
  - [ ] OK    Fix: 

- **`snapshotRecallFailed`**
  - EN: Snapshot '{name}' could not be recalled: {error}
  - FR: Le snapshot « {name} » n'a pas pu être rappelé : {error}
  - [ ] OK    Fix: 

- **`snapshotScopeUpdated`**
  - EN: Snapshot '{name}' scope updated.
  - FR: Portée du snapshot '{name}' mise à jour.
  - [ ] OK    Fix: 
...
- **`secondaryTouch`**
  ...
  - FR: Suspendre ou rétablir les modifications au deuxième doigt : rotation et hauteur des entrées, image stéréo, rotation et échelle des clusters. Le panoramique et le zoom restent actifs. Non enregistré.
  - [ ] OK    Fix: 
...
- **`trackingConflictsHeader`**
  ...
  - FR: Les clusters suivants ont plusieurs entrées avec le suivi activé :
  - [ ] OK    Fix: 
...
- **`admAxisSwap`**
  ...
  - FR: Quel axe ADM-OSC entrant correspond à cet axe interne.
  - [ ] OK    Fix: 

- **`admAzFlip`**
  - EN: Invert the direction of incoming azimuth.
  - FR: Inverser la direction de l'azimut entrant.
  - [ ] OK    Fix: 

- **`admAzOffset`**
  - EN: Azimuth offset (deg) applied to incoming ADM-OSC azimuth.
  - FR: Offset d'azimut (degrés) appliqué à l'azimut ADM-OSC entrant.
  - [ ] OK    Fix: 

- **`admBreakpoint`**
  - EN: Normalized breakpoint (0-1) for piecewise linear stretch.
  - FR: Point de rupture normalisé (0-1) pour l'étirement linéaire par morceaux.
  - [ ] OK    Fix: 

- **`admCenterOffset`**
  - EN: Physical position (m) where normalized 0.0 maps to.
  - FR: Position physique (m) où la valeur normalisée 0.0 est assignée.
  - [ ] OK    Fix: 

- **`admDistMax`**
  - EN: Maximum physical distance (m) at ADM-OSC distance=1.
  - FR: Distance physique maximale (m) à ADM-OSC distance=1.
  - [ ] OK    Fix: 

- **`admDistMin`**
  - EN: Minimum physical distance (m) at ADM-OSC distance=0.
  - FR: Distance physique minimale (m) à ADM-OSC distance=0.
  - [ ] OK    Fix: 

- **`admElFlip`**
  - EN: Invert the sign of incoming elevation.
  - FR: Inverser le signe de l'élévation entrante.
  - [ ] OK    Fix: 

- **`admInnerWidth`**
  - EN: Physical extent (m) from center to breakpoint.
  - FR: Étendue physique (m) du centre au point de rupture.
  - [ ] OK    Fix: 

- **`admInputAssign`**
  - EN: Assign this input to an ADM-OSC mapping for receive/transmit.
  - FR: Assigner cette entrée à une assignation ADM-OSC pour réception/transmission.
  - [ ] OK    Fix: 

- **`admLinkAll`**
  - EN: Select all 6 sides at once for uniform editing.
  - FR: Sélectionner les 6 côtés simultanément pour une édition uniforme.
  - [ ] OK    Fix: 
...
- **`admOuterWidth`**
  ...
  - FR: Étendue physique (m) du point de rupture à ±1.
  - [ ] OK    Fix: 

- **`admSideSelect`**
  - EN: Select sides to edit. Changes apply to all selected sides at once.
  - FR: Sélectionner les côtés à éditer. Les modifications s'appliquent à tous les côtés sélectionnés simultanément.
  - [ ] OK    Fix: 

- **`admSignFlip`**
  - EN: Invert the sign of the incoming axis value.
  - FR: Inverser le signe de la valeur de l'axe entrant.
  - [ ] OK    Fix: 
...
- **`networkInterface`**
  ...
  - FR: Sélectionner l'Network Interface.
  - [ ] OK    Fix: 
...
- **`trackingOscPath`**
  ...
  - FR: OSC Path pour le Tracking en mode OSC (commence par /)
  - [ ] OK    Fix: 
...
- **`notResponding`**
  ...
  - FR: La télécommande ne répond pas — vérifiez l'IP/le port de la tablette, et que WFS Control est lancé et à jour
  - [ ] OK    Fix: 

- **`protocolMismatch`**
  - EN: Remote app uses protocol v{remote}, expected v{local} — update the tablet app
  - FR: L'application distante utilise le protocole v{remote}, v{local} attendu — mettez à jour l'application tablette
  - [ ] OK    Fix: 
...
- **`sendFailing`**
  ...
  - FR: L'envoi vers la tablette échoue — sur macOS, autorisez WFS-DIY dans Réglages Système › Confidentialité et sécurité › Réseau local, et vérifiez que cet ordinateur est sur le réseau de la tablette
  - [ ] OK    Fix: 
...
- **`arrayMute`**
  ...
  - FR: Couper ou rétablir tous les haut-parleurs du groupe de cette sortie. Valable pour la session seulement : non enregistré avec le projet et annulé au chargement d'un projet. Indisponible pour les sorties Single.
  - [ ] OK    Fix: 
...
- **`eqFreq`**
  ...
  - FR: Fréquence Output EQ bande {band} (20 Hz - 20 kHz).
  - [ ] OK    Fix: 

- **`eqGain`**
  - EN: Output EQ Band {band} gain (-24 to +24 dB).
  - FR: Gain Output EQ bande {band} (-24 à +24 dB).
  - [ ] OK    Fix: 

- **`eqQ`**
  - EN: Output EQ Band {band} Q factor (0.1 - 10.0).
  - FR: Facteur Q Output EQ bande {band} (0.1 - 10.0).
  - [ ] OK    Fix: 
...
- **`eqShape`**
  ...
  - FR: Forme du filtre Output EQ bande {band}.
  - [ ] OK    Fix: 
...
- **`floorReflections`**
  ...
  - FR: Autoriser ou exclure cette enceinte des Floor Reflections.
  - [ ] OK    Fix: 
...
- **`liveSource`**
  ...
  - FR: Autoriser ou exclure cette enceinte de l'atténuation Live Source. (peut affecter le reste du groupe)
  - [ ] OK    Fix: 
...
- **`minLatency`**
  ...
  - FR: Autoriser ou exclure cette enceinte du traitement Minimal Latency. (peut affecter le reste du groupe)
  - [ ] OK    Fix: 
...
- **`arrayEditSingle`**
  ...
  - FR: Édition Ctrl : modification appliquée à cette sortie uniquement (Groupe {array} non affecté)
  - [ ] OK    Fix: 

- **`arrayMuted`**
  - EN: Array {array} muted
  - FR: Groupe {array} coupé
  - [ ] OK    Fix: 

- **`arrayUnmuted`**
  - EN: Array {array} unmuted
  - FR: Groupe {array} rétabli
  - [ ] OK    Fix: 
...
- **`configNotFound`**
  ...
  - FR: Fichier de configuration des sorties introuvable.
  - [ ] OK    Fix: 

- **`configReloaded`**
  - EN: Output configuration reloaded.
  - FR: Configuration des sorties rechargée.
  - [ ] OK    Fix: 
...
- **`noBackup`**
  ...
  - FR: Aucune sauvegarde de configuration des sorties trouvée.
  - [ ] OK    Fix: 
...
- **`algoFDNGpu`**
  ...
  - FR: Exécute la réverb FDN sur le GPU (ajoute ~20 ms au signal wet uniquement) ou sur le CPU. Bascule automatiquement sur le CPU si le GPU est indisponible.
  - [ ] OK    Fix: 
...
- **`algoIRGpu`**
  ...
  - FR: Exécute la convolution IR sur le GPU (ajoute ~20 ms au signal wet uniquement) ou sur le CPU. Bascule automatiquement sur le CPU si le GPU est indisponible.
  - [ ] OK    Fix: 
...
- **`algoSDNGpu`**
  ...
  - FR: Exécute la réverb SDN sur le GPU (ajoute ~20 ms au signal wet uniquement) ou sur le CPU. Bascule automatiquement sur le CPU si le GPU est indisponible.
  - [ ] OK    Fix: 
...
- **`applyToAllNodes`**
  ...
  - FR: Lorsque activé, les modifications de paramètres s'appliquent à tous les nœuds de réverbération à la fois ; lorsque désactivé, uniquement au nœud sélectionné. Effet à l'exécution uniquement - seules les modifications futures se propagent.
  - [ ] OK    Fix: 
...
- **`distanceAtten`**
  ...
  - FR: Distance attenuation pour le retour de réverbération (-6.0 à 0.0 dB/m).
  - [ ] OK    Fix: 

- **`distanceAttenEnable`**
  - EN: Distance attenuation percentage (0-200%).
  - FR: Pourcentage de Distance attenuation (0-200%).
  - [ ] OK    Fix: 
...
- **`mapVisibility`**
  ...
  - FR: Rendre visibles ou masquer tous les Reverb Channels sur la carte.
  - [ ] OK    Fix: 

- **`miniLatencyTooltip`**
  - EN: Allow or exclude this reverb feed from Minimal Latency processing.
  - FR: Autoriser ou exclure ce départ de réverbération du traitement Minimal Latency.
  - [ ] OK    Fix: 
...
- **`conflict`**
  ...
  - FR: Utilisée par « {name} » (canal {ch}, note {note}) — choisissez une autre note pour enregistrer.
  - [ ] OK    Fix: 
...
- **`deleteMessage`**
  ...
  - FR: Supprimer le canal d’entrée {number} ? Ses réglages sont perdus. Sur une session neuve, les canaux restants sont renumérotés selon l’ordre d’affichage ; dès que la session est utilisée, son numéro reste retiré en laissant un trou et les autres gardent les leurs, donc le patch, les snapshots, les cues QLab et les affectations DAW restent valides.
  - [ ] OK    Fix: 

- **`deleteTitle`**
  - EN: Remove Input Channel
  - FR: Supprimer le canal d’entrée
  - [ ] OK    Fix: 

- **`dragHint`**
  - EN: Drag a channel to change the order.
  - FR: Glissez un canal pour changer l’ordre.
  - [ ] OK    Fix: 
...
- **`dragHintFresh`**
  ...
  - FR: Session neuve : cet ordre devient la numérotation des canaux, et le patch est disposé pour y correspondre. Placez les canaux stéréo et mono maintenant, avant toute sauvegarde ou tout patch.
  - [ ] OK    Fix: 
...
- **`dragHintInUse`**
  ...
  - FR: Session en cours : les numéros sont permanents. Réordonner ne change que l'affichage — chaque canal garde ses entrées matérielles, et les snapshots, cues et télécommandes continuent de viser le même canal. La matrice de patch ne se lira plus comme une diagonale.
  - [ ] OK    Fix: 
...
- **`fromFileHelp`**
  ...
  - FR: Adopter les numéros ou l'ordre des canaux d'une config système ou d'entrées sauvegardée, sans rien charger d'autre.
  - [ ] OK    Fix: 

- **`fromFileTitle`**
  - EN: Take the channel list from a saved config
  - FR: Reprendre la liste des canaux d'une config sauvegardée
  - [ ] OK    Fix: 
...
- **`title`**
  ...
  - FR: Organiser les canaux d’entrée
  - [ ] OK    Fix: 
...
- **`channelIdentity.alreadyMatches`**
  ...
  - FR: La liste des canaux correspond déjà à {file}.
  - [ ] OK    Fix: 

- **`channelIdentity.cancelled`**
  - EN: Load cancelled: channel list not reconciled.
  - FR: Chargement annulé : liste des canaux non réconciliée.
  - [ ] OK    Fix: 
...
- **`channelIdentity.conflictOnly`**
  ...
  - FR: Les listes sont en conflit, rien ne peut être repris sans risque. Charger ce fichier ferait ceci :
  - [ ] OK    Fix: 

- **`channelIdentity.fixFailed`**
  - EN: Could not reconcile the channel list: {error}
  - FR: Impossible de réconcilier la liste des canaux : {error}
  - [ ] OK    Fix: 

- **`channelIdentity.fixed`**
  - EN: Channel list now matches {file}.
  - FR: La liste des canaux correspond maintenant à {file}.
  - [ ] OK    Fix: 

- **`channelIdentity.intro`**
  - EN: {file} describes a different channel list than this session.
  - FR: {file} décrit une liste de canaux différente de cette session.
  - [ ] OK    Fix: 

- **`channelIdentity.lineAdded`**
  - EN:   #{number} ({type}) is created with default settings
  - FR:   #{number} ({type}) est créé avec les réglages par défaut
  - [ ] OK    Fix: 
...
- **`channelIdentity.linePatch`**
  ...
  - FR:   {channel} : sauvegardé sur l'entrée {file}, maintenant sur {live}
  - [ ] OK    Fix: 

- **`channelIdentity.lineRemoved`**
  - EN:   {channel} is removed with its settings
  - FR:   {channel} est supprimé avec ses réglages
  - [ ] OK    Fix: 

- **`channelIdentity.lineRetyped`**
  - EN:   {channel} becomes {newType} and keeps its current settings
  - FR:   {channel} devient {newType} et garde ses réglages actuels
  - [ ] OK    Fix: 
...
- **`channelIdentity.lineRowsCross`**
  ...
  - FR:   les entrées matérielles restent à leur position de ligne, donc chaque canal déplacé reçoit les entrées d'un autre
  - [ ] OK    Fix: 

- **`channelIdentity.loadAnyway`**
  - EN: Load anyway applies the file's list by channel NUMBER:
  - FR: Charger quand même applique la liste du fichier par NUMÉRO de canal :
  - [ ] OK    Fix: 

- **`channelIdentity.loadAnywayButton`**
  - EN: Load anyway
  - FR: Charger quand même
  - [ ] OK    Fix: 
...
- **`channelIdentity.noIdentity`**
  ...
  - FR: {file} a été sauvegardé par une version antérieure et ne contient qu'un nombre de canaux ({count}) ; cette session en a {liveCount}. La liste actuelle est conservée et le patch du fichier est appliqué par position de ligne ; {delta}.
  - [ ] OK    Fix: 

- **`channelIdentity.noIdentityAppend`**
  - EN: {n} mono channel(s) are appended
  - FR: {n} canal(aux) mono sont ajoutés
  - [ ] OK    Fix: 

- **`channelIdentity.noIdentityKeep`**
  - EN: no channel is added or removed
  - FR: aucun canal n'est ajouté ni supprimé
  - [ ] OK    Fix: 

- **`channelIdentity.noIdentityRemove`**
  - EN: the {n} highest-numbered channel(s) are removed: {channels}
  - FR: le(s) {n} canal(aux) au numéro le plus élevé sont supprimés : {channels}
  - [ ] OK    Fix: 

- **`channelIdentity.noIdentityShort`**
  - EN: {file} carries no channel list to adopt (saved by an older version).
  - FR: {file} ne contient pas de liste de canaux à reprendre (version antérieure).
  - [ ] OK    Fix: 

- **`channelIdentity.notAConfig`**
  - EN: {file} is not a system or input config file.
  - FR: {file} n'est pas un fichier de config système ou d'entrées.
  - [ ] OK    Fix: 

- **`channelIdentity.notUndoable`**
  - EN: None of this can be undone, and it clears the undo history of every tab.
  - FR: Rien de tout cela n'est annulable, et l'historique d'annulation de tous les onglets sera effacé.
  - [ ] OK    Fix: 
...
- **`channelIdentity.pairAdvice`**
  ...
  - FR: Cela arrive quand l'un des deux a été enregistré seul. Annulez et enregistrez une fois la configuration complète pour les remettre en accord.
  - [ ] OK    Fix: 

- **`channelIdentity.pairLine`**
  - EN:   {channel} becomes #{newNumber}
  - FR:   {channel} devient #{newNumber}
  - [ ] OK    Fix: 
...
- **`channelIdentity.pairMismatch`**
  ...
  - FR: system.xml et inputs.xml de ce projet ne sont pas d'accord sur la liste des canaux. system.xml décide des numéros, des types et du patch ; inputs.xml décide ensuite des réglages, des types et de l'ordre :
  - [ ] OK    Fix: 

- **`channelIdentity.patchOnly`**
  - EN: Same channel list, but it was saved under a different patching:
  - FR: Même liste de canaux, mais sauvegardée avec un autre patch :
  - [ ] OK    Fix: 
...
- **`channelIdentity.relabelOffer`**
  ...
  - FR: L'arrangement correspond position par position — les mêmes canaux mono et stéréo aux mêmes places — mais les numéros diffèrent.\n\nReprendre les numéros du fichier : chaque canal garde sa place, ses réglages et ses entrées matérielles, et reçoit le numéro du fichier pour cette position :
  - [ ] OK    Fix: 
...
- **`channelIdentity.relabelOfferHw`**
  ...
  - FR: D'après les entrées matérielles, ces canaux semblent être les mêmes sous d'autres numéros.\n\nReprendre les numéros du fichier : chaque canal garde sa place, ses réglages et ses entrées matérielles ; seul son numéro change, afin que les snapshots, cues et OSC utilisant les numéros du fichier atteignent les bons canaux :
  - [ ] OK    Fix: 
...
- **`channelIdentity.reorderOffer`**
  ...
  - FR: Les mêmes canaux sont dans un ordre différent.\n\nRéordonner d'abord : les canaux sont placés dans l'ordre du fichier et gardent leurs numéros, réglages et entrées matérielles.
  - [ ] OK    Fix: 

- **`channelIdentity.stillDiffers`**
  - EN: The channel list still differs from {file} after reconciling; load cancelled.
  - FR: La liste des canaux diffère encore de {file} après réconciliation ; chargement annulé.
  - [ ] OK    Fix: 

- **`channelIdentity.takeNumbers`**
  - EN: Take the file's numbers
  - FR: Reprendre les numéros du fichier
  - [ ] OK    Fix: 

- **`channelIdentity.takeNumbersHw`**
  - EN: Take the file's numbers (by hardware input)
  - FR: Reprendre les numéros du fichier (par entrée matérielle)
  - [ ] OK    Fix: 

- **`channelIdentity.title`**
  - EN: Channel list differs from the file
  - FR: La liste des canaux diffère du fichier
  - [ ] OK    Fix: 
...
- **`reduceOutputChannels.title`**
  ...
  - FR: Réduire les Output Channels ?
  - [ ] OK    Fix: 
...
- **`reduceReverbChannels.title`**
  ...
  - FR: Réduire les Reverb Channels ?
  - [ ] OK    Fix: 
...
- **`dialsAndButtons`**
  ...
  - FR: Sélectionner le contrôleur matériel pour les boutons et molettes : Stream Deck+.
  - [ ] OK    Fix: 
...
- **`editChannels`**
  ...
  - FR: Organiser les canaux d’entrée : glissez pour entrelacer mono et stéréo dans n’importe quel ordre, ou supprimez un canal précis. Sur une session neuve, les numéros suivent l’ordre d’affichage ; ils deviennent permanents dès que la session est utilisée — projet chargé, onglet Inputs ou Map ouvert, fenêtre de patch ouverte, snapshot enregistré ou contrôle externe connecté. À partir de là, un canal garde son numéro où que vous le glissiez et un numéro supprimé reste retiré en laissant un trou — patch, snapshots, cues QLab et affectations du plug-in DAW ne cassent jamais. Verrouillé quand le DSP tourne.
  - [ ] OK    Fix: 
...
- **`gpuPipelineDepth`**
  ...
  - FR: Profondeur du pipeline GPU en blocs audio. Ajoute depth x buffer/fréquence d'échantillonnage de latence constante (pré-soustraite des délais WFS) et absorbe les interruptions GPU de même durée : plus profond = immunisé contre les à-coups du bureau/de l'interface, moins profond = latence plus faible. Application en direct. Défaut 4.
  - [ ] OK    Fix: 
...
- **`inputChannels`**
  ...
  - FR: Nombre d’entrées mono. Augmenter ajoute des canaux après le dernier ; réduire supprime les canaux mono aux numéros les plus élevés. Verrouillé quand le DSP tourne.
  - [ ] OK    Fix: 
...
- **`masterLevel`**
  ...
  - FR: Master Level (affecte toutes les sorties).
  - [ ] OK    Fix: 
...
- **`positionControl`**
  ...
  - FR: Sélectionner le contrôleur matériel pour le contrôle de position : Space Mouse, Joystick ou manette de jeu. Avec une Space Mouse, maintenir Maj sur une entrée stéréo transforme la torsion en axe stéréo et pousser/tirer en largeur stéréo.
  - [ ] OK    Fix: 
...
- **`speedOfSound`**
  ...
  - FR: Speed of Sound (liée à la température).
  - [ ] OK    Fix: 
...
- **`stereoInputChannels`**
  ...
  - FR: Nombre d’entrées stéréo (paires). Chaque paire garde un seul numéro de canal et occupe deux entrées matérielles (L et R) dans le patch. Augmenter ajoute après le dernier canal ; réduire supprime les stéréo aux numéros les plus élevés. Verrouillé quand le DSP tourne.
  - [ ] OK    Fix: 
