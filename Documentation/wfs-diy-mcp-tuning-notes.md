...
- **Channel create/delete** — `input_create` / `input_delete` / `output_create` / `output_delete` / `reverb_create` / `reverb_delete` ([Source/Network/MCP/tools/ChannelLifecycleTools.h](../Source/Network/MCP/tools/ChannelLifecycleTools.h)). Tier 2 (each call only adjusts the count by 1, vs the auto-gen tier-3 setter that takes arbitrary counts). Returns the new 1-based channel id; refused with `at_capacity` (64 in, 128 out, 32 reverb) or `empty`. (`8ec4945`)
