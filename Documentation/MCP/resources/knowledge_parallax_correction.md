...
- The stage is raised, and the audience sits at a lower elevation.
- Seats are tiered; back rows are elevated.
- Flown speakers are above the stage, aimed at mid-to-back rows meters away.
...
- **Vertical distance**: the height offset. Positive when the speaker is above the listener (flown array); negative when the speaker is below the listener (unusual but possible).
...
- **Lower array speakers** aimed at 2 m horizontal, 1.2 m vertical target (first row seated at ~2 m from the speakers, 1.2 m below the speaker if the speakers are on the stage edge).
- **Flown array speakers** aimed at 6 m horizontal, 0 m vertical (middle of the audience, at ear height) or adjusted for the specific seating geometry.
