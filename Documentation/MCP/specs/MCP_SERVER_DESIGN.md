...
| `Source/Automation/AutomOtionProcessor.h` | `Move` | Same shape as LFO. The tag value `Move` rather than `Automation` is intentional — automation refers to DAW host automation arriving via the plugin layer, which sets `Automation`. |
